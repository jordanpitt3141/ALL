...
%define w,b
To describe the FEVM we first present an overview of the evolution step and then provide the details for each component. We begin our evolution step given all the cell averages for the height of the water above the bed $h$, the absolute location of the free-surface $w$ and the conserved quantity $G$ at time $t^n$ and all the nodal values of the bed profile $b$. We write these as vectors from the starting $0^{th}$ to the final $m^{th}$ cell in the following way
...
\begin{enumerate}[(i)]
	...
	\end{bmatrix}
	\end{align*}
	where $q$ is a generic quantity demonstrating these vectors for $h$, $w$ and $G$. Note that since the domain contains all the cells from the $0^{th}$ cell to the $m^{th}$ cell that $q^+_{-1/2}$ and $q^-_{m+1/2}$ are the reconstructed values of $q$ at the left and right boundaries of the domain, respectively.	
...
Integrating this equation by parts with zero Dirichlet boundary conditions we get
\begin{multline}
...
\end{equation}
This transformation is ill-defined when $h = 0$ so we also add in a  small term $h_{tol}$ to the denominator. The term $h_{tol}$ serves as our cut-off value with any cells with $h < h_{tol}$ being considered dry. Therefore, our transformation for the reconstructed values of $h$ in the finite element method is
\begin{subequations}
	\begin{align}
