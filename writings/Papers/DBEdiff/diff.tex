...
%DIF DEL ../mDBEclean/DamBreak.tex   Tue Jun 27 18:22:40 2017
%DIF ADD ../DBEnew/DamBreak.tex      Tue Aug 29 13:22:11 2017
...
\DIFdelbegin \DIFdel{Expressions for the }\DIFdelend \DIFaddbegin 

\DIFadd{Utilising modulation theory expressions for the long term }\DIFaddend leading wave amplitude and speed of an undular bore for the Serre equations were derived and verified for a range of undular bores by \citet{El-etal-2006}. These expressions were also shown to be valid for all the different structures found in the literature \cite{El-etal-2006,Mitsotakis-etal-2014}.


%Aim
The first aim of this paper is to investigate and explain why different \DIFaddbegin \DIFadd{short term }\DIFaddend behaviour has been published in the literature for numerical solutions of the Serre equations for problems containing steep gradients. We find that the undulations of a bore can be damped to a constant depth and velocity state by the numerical diffusion introduced by the method, as is the case for \citet{Hank-etal-2010-2034}. Oscillation damping can also occur due to the particular smoothing of the initial conditions, as is the case for \citet{Mitsotakis-etal-2017}, \citet{El-etal-2006} and \citet{Mitsotakis-etal-2014}. \DIFdelbegin \DIFdel{We do find that }\DIFdelend \DIFaddbegin \DIFadd{Our results demonstrate that in the short term our `growth' structure is the structure that should be observed for the solution of the dam-break problem for the Serre equations. While }\DIFaddend over long time periods the Serre equations \DIFdelbegin \DIFdel{damp these oscillations as they propagate, but }\DIFdelend \DIFaddbegin \DIFadd{damps this `growth' structure, which is consistent with the expected long term behaviour. However, }\DIFaddend this natural decay is dominated by other factors in the literature \DIFaddbegin \DIFadd{for numerical solutions over short time spans}\DIFaddend .

The second aim of this paper is to assess the utility of the shallow water wave equations \DIFdelbegin \DIFdel{and the results of \mbox{%DIFAUXCMD
\citet{El-etal-2006} }%DIFAUXCMD
as guides }\DIFdelend \DIFaddbegin \DIFadd{as a guide }\DIFaddend for the evolution of an undular bore \DIFaddbegin \DIFadd{and compare our numerical solutions with the Whitham modulation results of \mbox{%DIFAUXCMD
\citet{El-etal-2006}}%DIFAUXCMD
}\DIFaddend . We find that for a range of dam-break problems the analytical solution of the shallow water wave equations is a good approximation for the mean depth and velocity of the \DIFaddbegin \DIFadd{numerical solution of the }\DIFaddend Serre equations, \DIFdelbegin \DIFdel{extending the findings of \mbox{%DIFAUXCMD
\citet{Hank-etal-2010-2034} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Mitsotakis-etal-2014} }%DIFAUXCMD
to a larger range of dam-break problems}\DIFdelend \DIFaddbegin \DIFadd{as is in the literature \mbox{%DIFAUXCMD
\cite{Hank-etal-2010-2034,Mitsotakis-etal-2014}}%DIFAUXCMD
}\DIFaddend . It was also found that the results of \citet{El-etal-2006} are a good approximation to our numerical solutions. \DIFaddbegin \DIFadd{However, unlike the results of \mbox{%DIFAUXCMD
\citet{El-etal-2006} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Mitsotakis-etal-2014} }%DIFAUXCMD
we demonstrate that the Whitham modulation results can underestimate the leading wave height and speed in the near term, even when the ratio of the two water depths is below the critical value.
}\DIFaddend 
...
The paper is organised as follows, in Section \ref{section:Serre Equations} the Serre equations and the quantities they conserve are presented. In Section \ref{section:smootheddambreak} the smoothed dam-break problem is defined \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and the notation for }\DIFaddend the measures of the relative difference between numerical solutions and the relative error in the conserved quantities \DIFdelbegin \DIFdel{are presented. The }\DIFdelend \DIFaddbegin \DIFadd{is introduced. An }\DIFaddend analytical solution of the shallow water wave equations and the \DIFdelbegin \DIFdel{expressions }\DIFdelend \DIFaddbegin \DIFadd{notation }\DIFaddend for the amplitude and speed of the leading wave of an undular bore are presented. In Section \ref{sec:nummeth} the numerical methods and their important properties are presented. In Section \ref{section:NumRes} the four different structures in the solutions of smoothed dam-break problem for the Serre equations are determined using verified numerical solutions. The verified numerical solutions are also used to \DIFdelbegin \DIFdel{evaluate how well }\DIFdelend \DIFaddbegin \DIFadd{compare }\DIFaddend the analytical solution of the shallow water wave equations \DIFdelbegin \DIFdel{captures }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the mean behaviour of the solution of the Serre equations for the dam-break problem. The Whitham modulations results are also compared to the verified numerical solutions\DIFdelbegin \DIFdel{to test their veracity}\DIFdelend .
...
%DIFDELCMD < %%%
\begin{align*}
\DIFdel{%DIFDELCMD < \label{eqn:Condef}%%%
...
There are no known analytical solutions of the Serre equations for the dam-break problem or an arbitrary smoothed dam-break problem. Therefore, to demonstrate that our numerical solutions converge we use the relative difference between numerical solutions \DIFdelbegin \DIFdel{. To demonstrate that our numerical solutions also have small errors in the conserved quantities we use the relative error of their conservation. Both of these measures are defined in this section. }%DIFDELCMD < \begin{figure}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.6\textwidth]{Figure-1.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Initial conditions for smooth dam-break problems with $h_0 = 1m$, $h_1 = 1.8m$, $x_0 =500m$ and various $\alpha$.}}
%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:dbsmoothinit}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
%DIF < 
\subsection{\DIFdel{Assessing validity of Numerical Solutions}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$L^h_{1}$ and $L^u_{1}$ for the primitive variables $h$ and $u$ respectively. }\DIFaddend To demonstrate that our numerical solutions \DIFdelbegin \DIFdel{converge to a solution with little error in the conserved quantities as the spatial resolution is increased we use two measures; the relative difference between numerical solutions of different resolutions and the error in the conservation of a quantity. The relative difference between numerical solutions measures their convergence, while the error in conservation measures how well the numerical solutions conserve the quantities $h$, $uh$ and $\mathcal{H}$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We introduce the following notation for the spatial grids defined by $x_i$ and the temporal grids defined by $t^n$ upon which the numerical solutions are calculated. These grids are uniform so that $\Delta x = x_{i} - x_{i-1}$ for all $ i$ and $\Delta t = t^{n} - t^{n-1}$ for all $n$. We use subscripts and superscripts to denote where a quantity $q$ is evaluated in the following way $q_i^n = q(x_i,t^n)$. Finally, the $i$th cell is the interval }%DIFDELCMD < [%%%
\DIFdel{$x_i -\Delta x/2$}\DIFdelend \DIFaddbegin \DIFadd{conserve the quantities $h$}\DIFaddend , \DIFdelbegin \DIFdel{$x_i +\Delta x/2$}%DIFDELCMD < ] %%%
\DIFdel{centred around $x_{i}$}\DIFdelend \DIFaddbegin \DIFadd{$uh$ and $\mathcal{H}$ well we use the relative error of their conservation $C_1^{h}$, $C_1^{uh}$ and $C_1^{\mathcal{H}}$ respectively}\DIFaddend .

\DIFdelbegin \subsubsection{\DIFdel{Convergence of Numerical Results}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdel{To measure the convergence of the numerical solutions we ensured all grids had common locations to compare them by dividing $\Delta x$ by $2$ to create finer grids. Therefore, the finest grid with the smallest $\Delta x$ contains all the locations $x_i$ in any coarser grid. To measure the relative difference between quantities on these grids we compare them only on the coarser grid points $x_i$. For some quantity $q$ we have our numerical approximation to it on the finest grid $q^*$ and on the coarser grid $q'$, with the relative difference between the two being
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < 	%%%
\begin{align*}
	\DIFdel{L_1^{q} = \dfrac{\sum_{i} \left| q'(x_i)  - q^*(x_i)\right|}{\sum_{i} \left| q^*(x_i)\right|}.
	%DIFDELCMD < \label{eq:L1def}%%%
	}\end{align*}
%DIFAUXCMD
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsubsection{\DIFdel{Conserved Quantities}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdel{To calculate the error in conservation of a quantity, we must first calculate the total amount of the conserved quantities for the initial conditions. For the smoothed }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Background for derived and observed comparisons}}
\DIFadd{Because there are no known analytic solutions to the }\DIFaddend dam-break problem \DIFdelbegin \DIFdel{the initial conditions }%DIFDELCMD < \eqref{eq:sdbi} %%%
\DIFdel{were integrated to obtain expressions for the total mass $C_{h}(0)$, the total momentum $C_{uh}(0)$ and the total Hamiltonian $C_{\mathcal{H}}(0)$. Provided $x_0$ is the midpoint of the spatial domain $\left[a,b \right]$ the total amounts for the conserved quantities are
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < 	\begin{subequations}
%DIFDELCMD < 		%%%
\begin{align*}
		\DIFdel{\mathcal{C}_{h}(0) = \frac{h_1 + h_0}{2}\left(b- a\right),
		%DIFDELCMD < \label{eq:Chdef}%%%
		}\end{align*}
		%DIFAUXCMD
\begin{align*}
		\DIFdel{\mathcal{C}_{uh}(0) = 0
		%DIFDELCMD < \label{eq:Cuhdef}%%%
		}\end{align*}
		%DIFAUXCMD
\DIFdel{and
		}\begin{align*}
		\DIFdel{\mathcal{C}_{\mathcal{H}}(0) = \frac{g}{4} \left(h_0^2 - h_1^2 + \alpha\left(h_1 - h_0\right)^2\tanh\left(\frac{a - b}{2 \alpha}\right)\right).
		%DIFDELCMD < \label{eq:CHdef}%%%
		}\end{align*}
		%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{eq:Canalyticvalues}	%%%
%DIFDELCMD < 	\end{subequations}
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To calculate how well we approximate the total amount of a quantity $q$ in our numerical solution we fit a quartic interpolant of the primitive variables $h$ and $u$ over a cell utilising neighbouring cells and then apply Gaussian quadrature with 3 points. The amount of $q$ in each cell is summed across all cells to get the total amount of $q$ in our numerical solution at time $t$, which we call $\mathcal{C}^*_{q}(t)$. The error in conservation of a quantity $q$ for a numerical solution is
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < 	%%%
\begin{align*}
	\DIFdel{C_1^q = \frac{\left| \mathcal{C}_{q}(0) - \mathcal{C}^*_{q}(t) \right| }{\left|\mathcal{C}_{q}(0)\right|}.
	}\end{align*}
%DIFAUXCMD
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < %%%
\DIFdel{Note that for $uh$ the denominator is $0$ and that there is a flux of momentum due to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the \DIFdelbegin \DIFdel{unequal heights at both ends of the domain. To resolve this issue for $uh$ the error in the conservation of $uh$ is measured by
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < 	%%%
\begin{align*}
	\DIFdel{C_1^{uh} = \left| \mathcal{C}_{uh}(0) - \mathcal{C}^*_{uh}(t) - \frac{gt}{2}\left(h(b)^2 - h(a)^2\right)\right|  .
	%DIFDELCMD < \label{eq:C1def}%%%
	}\end{align*}
%DIFAUXCMD
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Serre equations the literature has relied on comparisons to simpler equations such as the shallow water wave equations \mbox{%DIFAUXCMD
\cite{Hank-etal-2010-2034,Mitsotakis-etal-2014} }%DIFAUXCMD
or asymptotic solutions \mbox{%DIFAUXCMD
\cite{El-etal-2006} }%DIFAUXCMD
to understand the behaviour of the Serre equations for the dam-break problem.
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Background for derived and observed comparisons}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend It was demonstrated by \citet{Hank-etal-2010-2034} and \citet{Mitsotakis-etal-2014} that the analytical solution of the shallow water wave equations for the dam-break problem captures the mean behaviour of the numerical solutions of the Serre equations to the dam-break problem \cite{Hank-etal-2010-2034} and the smoothed dam-break problem \cite{Mitsotakis-etal-2014}. 
...
%DIFDELCMD < %%%
\DIFdel{To be self contained we present the analytical solution of the shallow water wave equations to the dam-break problem and the expressions derived by \mbox{%DIFAUXCMD
\citet{El-etal-2006}}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend An example of the analytical solution of the shallow water wave equations for the dam-break problem is presented in Figure \ref{fig:SWWEanadiagram}. Region I is the undisturbed water upstream of the dam-break at constant height ($h_1$) and velocity ($0m/s$). Region II is the rarefaction fan connecting regions I and III. Regions III and IV \DIFdelbegin \DIFdel{represent the shock with }\DIFdelend \DIFaddbegin \DIFadd{compose the }\DIFaddend constant height ($h_2$) and constant velocity ($u_2$) \DIFdelbegin \DIFdel{, these regions are }\DIFdelend \DIFaddbegin \DIFadd{state, with the regions }\DIFaddend separated by $x_{u_2} = x_0 + u_2t$. Region V is the undisturbed water downstream at constant height ($h_0$) and velocity ($0m/s$) separated from Region IV by a shock which travels at velocity $S_2$. Expressions for the unknown quantities $h_2$, $u_2$ and $S_2$ in terms of $h_0$ and $h_1$ \DIFdelbegin \DIFdel{were given by \mbox{%DIFAUXCMD
\citet{Wu-etal-1999-1210} }%DIFAUXCMD
as
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < \begin{subequations}
%DIFDELCMD < %%%
\begin{align*}
\DIFdel{h_2 = \frac{h_0}{2} \left(\sqrt{1 + 8 \left(\frac{2h_2}{h_2 - h_0}\frac{\sqrt{h_1} - \sqrt{h_2}}{\sqrt{h_0}}\right)^2} - 1\right),
}\end{align*}
	%DIFAUXCMD
\begin{align*}
	\DIFdel{u_2 = 2\left(\sqrt{gh_1} - \sqrt{gh_2}\right)
	}\end{align*}
%DIFAUXCMD
\DIFdel{and
	}\begin{align*}
	\DIFdel{S_2 = \frac{h_2 u_2}{h_2 - h_0}.
	}\end{align*}
%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{eq:WuSWWE}	%%%
%DIFDELCMD < \end{subequations}
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < %%%
\DIFdel{Applying }%DIFDELCMD < \eqref{eq:WuSWWE} %%%
\DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{are known, with the expressions of \mbox{%DIFAUXCMD
\citet{Wu-etal-1999-1210} }%DIFAUXCMD
requiring the solution of a single non-linear equation.
}

\DIFadd{For }\DIFaddend our dam-break heights of interest; $h_0 =1m$ and $h_1 = 1.8m$ \DIFdelbegin \DIFdel{results in }\DIFdelend \DIFaddbegin \DIFadd{we have }\DIFaddend $h_2 = 1.36898m$ , $u_2 = 1.074975$ $m/s$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $S_2 = 3.98835$ $m/s$ \DIFaddbegin \DIFadd{and $x_{u_2} = 532.24925m$ }\DIFaddend which are shown in Figure \ref{fig:SWWEanadiagram} for $t=30s$. The location of the front of the bore for the shallow water wave equations at time $t$ is thus \DIFdelbegin \DIFdel{$x_{S_2}(t) = x_0 + S_2 t$ so that $x_{S_2}(30s) = 619.6505m$}\DIFdelend \DIFaddbegin \DIFadd{$x_{S_2} = x_0 + S_2 t$ so that $x_{S_2} = 619.6505m$ at $t=30s$}\DIFaddend .
...
\DIFdelbegin \subsubsection{\DIFdel{Whitham Modulation for Undular Bores of the Serre Equations}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend Utilizing Whitham modulation theory for a one-phase periodic travelling wave \DIFdelbegin \DIFdel{an asymptotic analytical expression }\DIFdelend \DIFaddbegin \DIFadd{asymptotic expressions }\DIFaddend for the amplitude $A^+$ and speed $S^+$ of the leading wave \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend derived by \citet{El-etal-2006}. \DIFdelbegin \DIFdel{An example of an undular bore is shown in Figure \ref{fig:Serreanadiagram}.
The derived expressions for $A^+$ and $S^+$ are
}%DIFDELCMD < \begin{linenomath*}
%DIFDELCMD < 	\begin{subequations}
%DIFDELCMD < 		%%%
\begin{align*}
		\DIFdel{\frac{\Delta}{\left(A^+ + 1\right)^{1/4}} - \left(\frac{3}{4 -  \sqrt{A^+ + 1}}\right)^{21/10} \left(\frac{2}{1 + \sqrt{A^+ + 1}}\right)^{2/5} = 0
		%DIFDELCMD < \label{eq:aplusdef}%%%
		}\end{align*}
		%DIFAUXCMD
\DIFdel{and
		}\begin{align*}
		\DIFdel{S^+ = \sqrt{g \left(A^+ + 1\right)}
		%DIFDELCMD < \label{eq:splusdef}%%%
		}\end{align*}
		%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{eq:ELWhitMod}	%%%
%DIFDELCMD < 	\end{subequations}
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < %%%
\DIFdel{where $\Delta = h_b / h_0$, and $h_b$ is the height of the bore. The height of the bore created by the }\DIFdelend \DIFaddbegin \DIFadd{These expressions were found to be most appropriate for }\DIFaddend dam-break \DIFdelbegin \DIFdel{problem in }%DIFDELCMD < \eqref{eq:aplusdef} %%%
\DIFdel{used by \mbox{%DIFAUXCMD
\citet{El-etal-2006} }%DIFAUXCMD
was
}\begin{align*}
\DIFdel{%DIFDELCMD < \label{eqn:hrdef}%%%
h_b = \frac{1}{4}\left(\sqrt{\frac{h_1}{h_0}} + 1\right)^2.
}\end{align*} 
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{aspect ratios $h_1/ h_0 \le 1.94$.
}

\DIFaddend For our dam-break heights of interest $h_0 = 1m$ and $h_1 = 1.8m$ we obtain \DIFdelbegin \DIFdel{$h_b = 1.37082m$, $\Delta = 1.37082$, }\DIFdelend $A^+ = 1.73998m$ and $S^+ = 4.13148m/s$. The location of the leading wave of an undular bore at time $t$ is then \DIFdelbegin \DIFdel{$x_{S^+}(t) = x_0 + S^+ t$ so that $x_{S^+}(30s) = 623.9444m$}\DIFdelend \DIFaddbegin \DIFadd{$x_{S^+} = x_0 + S^+ t$ so that $x_{S^+} = 623.9444m$ for $t=30s$}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.5\textwidth]{Figure-3.pdf}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Demonstration of quantities obtained by Whitham modulation for undular bores of the Serre equations.}}
	%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:Serreanadiagram}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Numerical Methods}
\label{sec:nummeth}
...
%DIFDELCMD < %%%
\DIFdelend %[] include another zoomed plot []
\begin{figure}
	\centering
	\begin{subfigure}{\textwidth}
		\hspace{1mm}
...
The $\mathcal{S}_4$ ``growth'' structure, which has hitherto not been commonly published in the literature features a growth in the oscillation amplitude around $x_{u_2}$. \DIFaddbegin \DIFadd{This growth is a result of the interaction of the left and right wave trains, which were predicted by the linear theory of \mbox{%DIFAUXCMD
\citet{Dougalis-etal-2007} }%DIFAUXCMD
to be separate. 
}

%DIF > The linear theory of \citet{Dougalis-etal-2007} predicts the separation of these wave trains, and so this interaction is a result of non-linear effects. []

\DIFaddend An example of the growth structure can be seen for $\mathcal{V}_3$'s numerical solutions in Figure \ref{fig:o3a20dxlimcdexp} to the smoothed dam-break problem with $\alpha = 0.1m$. This structure was observed in the numerical solutions of $\mathcal{V}_3$ for $\Delta x = 10/2^{11}m$ at $t=30s$ for $\alpha$ values as low as $0.001m$ and even for the dam-break problem. \DIFaddbegin \DIFadd{The growth structure has also been observed using the finite element method of \mbox{%DIFAUXCMD
\citet{Mitsotakis-etal-2017-1}}%DIFAUXCMD
.
}\DIFaddend 
...
It can be seen that $h_2$ and $u_2$ are good approximations to the mean behaviour of the fluid inside the bore for a range of different aspect ratios. Although, as $h_1/h_0$ increases this approximation becomes worse, so that $h_2$ becomes an underestimate and consequently $u_2$ is an overestimate. \DIFaddbegin \DIFadd{Most likely this is due to the increasing influence of non-linearity on the undulations. 
}\DIFaddend 
...
The location of the leading wave of the Serre equations slowly diverges from the location of the front of a bore in the shallow water wave equations over long periods of time. This \DIFaddbegin \DIFadd{is because the Rankine Hugoniot conditions that determine the front of the bore for the shallow water wave equations are not applicable to dispersive shock waves \mbox{%DIFAUXCMD
\cite{El-Hoefer-2016-11}}%DIFAUXCMD
. This }\DIFaddend divergence causes the small difference evident in $\mathcal{V}_3$'s numerical solution to the smoothed dam-break problem with $\alpha =0.1m$ at $t=300s$ using $\Delta x = 10/2^{9}m$, which is shown in Figure \ref{fig:frontcomp}.
...
\citet{El-etal-2006} noted the presence of a `degenerate contact discontinuity' which is \DIFdelbegin \DIFdel{the node }\DIFdelend \DIFaddbegin \DIFadd{where the two wave trains interact }\DIFaddend in the $\mathcal{S}_3$ \DIFdelbegin \DIFdel{structure and }\DIFdelend \DIFaddbegin \DIFadd{and $\mathcal{S}_4$ structures; it }\DIFaddend travels at the mean fluid velocity in the bore.

We observe that as our numerical solutions evolve over time, oscillations appear to be released from the contact discontinuity and travel away from it in both directions, leading to decay of amplitudes around the contact discontinuity. Therefore, the contact discontinuity is an important feature and its behaviour determines the structure of the oscillations in \DIFdelbegin \DIFdel{the middle of the undular bore}\DIFdelend \DIFaddbegin \DIFadd{regions III and IV}\DIFaddend . 

The different speeds of the oscillations are determined by the phase velocity, which for the Serre equations linearised around the mean height $\bar{h}$ and mean velocity $\bar{u}$ in regions III and IV \DIFdelbegin \DIFdel{of the solution to the dam-break problem }\DIFdelend is
\begin{linenomath*}
	\begin{gather}
	\upsilon_p = \bar{u} \pm \sqrt{g\bar{h}} \sqrt{\frac{3}{\bar{h}^2 k^2 + 3}}
	\DIFaddbegin \label{eq:cd}
	\DIFaddend \end{gather}
\end{linenomath*}
with wave number $k$. It can be seen that as $k \rightarrow \infty$ then $\upsilon_p \rightarrow \bar{u}$ and as $k \rightarrow 0$ then $\upsilon_p \rightarrow \bar{u} \pm \sqrt{g\bar{h}}$. \DIFdelbegin \DIFdel{Since the contact discontinuity travels at the mean velocity inside the bore, it corresponds to very high wave number oscillations. The oscillations on the left travel slower than the contact discontinuity and are therefore lower wave number oscillations associated with }\DIFdelend \DIFaddbegin \DIFadd{The left wave train corresponds to $-$ branch of the phase velocity while the right wave train corresponds to the $+$ branch of }\DIFaddend the phase velocity\DIFdelbegin \DIFdel{$ \bar{u} - \sqrt{g\bar{h}} \sqrt{3/ (\bar{h}^2 k^2 + 3)}$. The oscillations on the right travel travel quicker than the contact discontinuity and are therefore lower wave number oscillations associated with the phase velocity $ \bar{u} + \sqrt{g\bar{h}} \sqrt{3/ (\bar{h}^2 k^2 + 3)}$. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{These different phase velocities have two different behaviours for $h$ and $u$. When }\DIFdelend \DIFaddbegin \DIFadd{. The contact discontinuity corresponds to the high wave-number limit of these two wave trains. Because the two wave trains correspond to the $-$ and $+$ branches of }\DIFaddend the phase velocity \DIFdelbegin \DIFdel{is $ \bar{u} + \sqrt{g\bar{h}} \sqrt{3/ (\bar{h}^2 k^2 + 3)}$ we have oscillations where }\DIFdelend \DIFaddbegin \DIFadd{for the left wave train }\DIFaddend $h$ and $u$ are in-phase, while \DIFdelbegin \DIFdel{when the phase velocity is $ \bar{u} - \sqrt{g\bar{h}} \sqrt{3/ (\bar{h}^2 k^2 + 3)}$ we have oscillations where }\DIFdelend \DIFaddbegin \DIFadd{for the right wave train }\DIFaddend $h$ and $u$ are out-of-phase. This can be seen in Figure \ref{fig:uhcomp} for the numerical solutions of $\mathcal{V}_3$ with $\Delta x = 10/2^{9}m$ for the smoothed dam-break problem with $\alpha = 0.1m$ at $t=30s$.

\DIFaddbegin \DIFadd{The $+$ branch of the phase velocity is strictly increasing in $k$, while the $-$ branch is strictly decreasing. It can also be shown that $+$ branch and $-$ branch of the group velocity behave in the same way with the same limits as their phase velocity counterparts. Therefore as in \mbox{%DIFAUXCMD
\citet{Dougalis-etal-2007} }%DIFAUXCMD
the linear theory predicts separate dispersive wave trains. However, we have demonstrated that in the short term these wave trains are not be seperate and so the interaction of these wave trains in the $\mathcal{S}_3$ and $\mathcal{S}_4$ structures is caused by non-linear effects.
%DIF > 
...
The Whitham modulation results for the location of the leading wave $x_{S^+}$ is a better approximation than that given by the shallow water wave equations $x_{S_2}$, as can be seen in Figure \ref{fig:frontcomp}. \DIFaddbegin \DIFadd{This is due to the inapplicability of the Rankine Hugoniot conditions from which $S_2$ is derived for dispersive shock waves \mbox{%DIFAUXCMD
\cite{El-Hoefer-2016-11}}%DIFAUXCMD
.
}

\DIFadd{The agreement between the Whitham modulation results and our numerical solutions is actually quite good given that the Whitham modulations results are an asymptotic solution to the dam-break problem. We include them here as a counter point to the results of \mbox{%DIFAUXCMD
\citet{El-etal-2006} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Mitsotakis-etal-2014}}%DIFAUXCMD
, as from their results one would expect that numerical solutions should only ever underestimate $A^+$ and $S^+$ when the ratio $h_1/h_0$ is less than the critical value $1.94$.
}\DIFaddend \begin{figure}
	\centering
...
We \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{confirm }\DIFaddend that the analytical solution of the shallow water wave equations for the dam-break problem provides a reasonable approximation to the mean height and velocity inside the bore formed by the smoothed dam-break problem for the Serre equations \DIFaddbegin \DIFadd{across a range of aspect ratios}\DIFaddend . Finally, we observe that the Whitham modulations results for the leading wave of an undular bore \DIFdelbegin \DIFdel{provide a more accurate approximation to the location and depth of the front of an undular bore than the shallow water wave equations}\DIFdelend \DIFaddbegin \DIFadd{can underestimate the leading wave amplitude and speed when the aspect ratio is below the critical value}\DIFaddend .

%more research\cite{Mitsotakis-etal-2016}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibliographystyle{elsarticle-num-names}
%--------------------------------------------------------------------------------
\DIFdelbegin %DIFDELCMD < \begin{thebibliography}{16}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{thebibliography}{18}
	...
	\DIFdelbegin %DIFDELCMD < \bibitem[{Mitsotakis et~al.(2017)Mitsotakis, Dutykh, and
%DIFDELCMD < 		Carter}]{Mitsotakis-etal-2017}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \bibitem[{Mitsotakis et~al.(2017{\natexlab{a}})Mitsotakis, Dutykh, and
		Carter}]{Mitsotakis-etal-2017}
	\DIFaddend \bibinfo{author}{D.~Mitsotakis}, \bibinfo{author}{D.~Dutykh},
	...
	(\DIFdelbegin %DIFDELCMD < \bibinfo{year}{2017}%%%
\DIFdelend \DIFaddbegin \bibinfo{year}{2017}{\natexlab{a}}\DIFaddend ) \bibinfo{pages}{166--182}.
	...
	\DIFaddbegin \bibitem[{El and Hoefer(2016)}]{El-Hoefer-2016-11}
	\bibinfo{author}{G.~El}\DIFadd{, }\bibinfo{author}{M.~Hoefer}\DIFadd{,
	}\bibinfo{title}{Dispersive shock waves and modulation theory}\DIFadd{,
	}\bibinfo{journal}{Physica D: Nonlinear Phenomena} \bibinfo{volume}{333}
	\DIFadd{(}\bibinfo{year}{2016}\DIFadd{) }\bibinfo{pages}{11--65}\DIFadd{.
	}

	\DIFaddend \bibitem[{El et~al.(2006)El, Grimshaw, and Smyth}]{El-etal-2006}
...
\DIFdelend \DIFaddbegin \bibitem[{Dougalis et~al.(2007)Dougalis, Duran, Lopez-Marcos, and
		Mitsotakis}]{Dougalis-etal-2007}
	\bibinfo{author}{V.~Dougalis}\DIFadd{, }\bibinfo{author}{A.~Duran}\DIFadd{,
	}\bibinfo{author}{M.~Lopez-Marcos}\DIFadd{, }\bibinfo{author}{D.~Mitsotakis}\DIFadd{,
	}\bibinfo{title}{Numerical study of the stability of solitary waves of the
		Bona--Smith family of Boussinesq systems}\DIFadd{, }\bibinfo{journal}{Journal of
		Nonlinear Science} \bibinfo{volume}{17}\DIFadd{~(}\bibinfo{number}{6}\DIFadd{)
	(}\bibinfo{year}{2007}\DIFadd{) }\bibinfo{pages}{569--607}\DIFadd{.
	}

	\bibitem[{Mitsotakis et~al.(2017{\natexlab{b}})Mitsotakis, Synolakis, and
		McGuinness}]{Mitsotakis-etal-2017-1}
	\bibinfo{author}{D.~Mitsotakis}\DIFadd{, }\bibinfo{author}{C.~Synolakis}\DIFadd{,
	}\bibinfo{author}{M.~McGuinness}\DIFadd{, }\bibinfo{title}{A modified Galerkin/finite
		element method for the numerical solution of the Serre-Green-Naghdi system}\DIFadd{,
	}\bibinfo{journal}{International Journal for Numerical Methods in Fluids}
	\bibinfo{volume}{83}\DIFadd{~(}\bibinfo{number}{10}\DIFadd{)
	(}\bibinfo{year}{2017}{\natexlab{b}}\DIFadd{) }\bibinfo{pages}{755--778}\DIFaddend .
