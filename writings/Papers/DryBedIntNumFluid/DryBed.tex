...
\corres{*J.P.A.~Pitt. \email{jordan.pitt@anu.edu.au}}
\author[ANU]{J.P.A.~Pit}
\author[ANU]{C.~Zoppou}
\author[ANU]{S.G.~Roberts}

\address[ANU]{\orgdiv{Mathematical Sciences Institute}, \orgname{Australian National University}, \orgaddress{\state{ACT}, \country{Australia}}}
...
These results demonstrate that the developed method has accurately reproduced the lake at rest solution and is therefore well-balanced. Without well-balancing modifications the numerical solution quickly develops non-zero velocities and a non-horizontal water surface profile as can be seen in Figure \ref{fig:WBnWBComp}.

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{./Figures/LakeAtRest/WBnWBComp.pdf}
	\caption{Numerical solution for $w$  with (\squareF{blue}) and without ({\color{red}\dashedrule}) well-balancing modifications over $\left[-25,0\right]$ where $\Delta x = {100} / {2^{10}}m $ for the lake at rest problem at $t=10s$.}
	\label{fig:WBnWBComp}
\end{figure}
