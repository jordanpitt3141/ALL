...
\end{gather}
\end{subequations}
\end{linenomath*} Then taking the second-order centred finite difference approximation to the time derivatives for \eqref{eq:expandedu} gives
\begin{linenomath*}
\begin{gather*}
...
\label{section:}
Because the conservation of mass equation \eqref{eq:Serre_continuity} has no mixed derivative term standard numerical techniques for conservation laws can be used. This paper will use two, one using the Lax Wendroff method as was done by \citeN{El-etal-2006} and the other will use the same finite difference approximation process as the above section. Both of these are theoretically second-order accurate. To make these methods precise they will be presented here in sufficient replicable detail.
...
%--------------------------------------------------------------------------------
To obtain a second-order finite difference approximation to \eqref{eq:Serre_continuity} the same procedure that was used above for \eqref{eq:Serre_momentum} was used. Thus the derivatives were first expanded then approximated by second order centered finite differences to give
\begin{linenomath*}
\begin{gather}
...
%--------------------------------------------------------------------------------
The conservative form \eqref{eq:Serrecon} allows for a wider range of numerical techniques such as finite element methods \cite{Guyenne-etal-2014-169} and finite volume methods \cite{Hank-etal-2010-2034,Zoppou-2014}. In this paper the first ($\mathcal{V}_1$), second ($\mathcal{V}_2$) and third-order ($\mathcal{V}_3$) finite difference-volume methods (FDVM) of [] will be used. These have been validated and their order of accuracy has been confirmed.

\subsection{Stability Condition} 
There is a restriction on the computational time-step that can be used for the FDVM used in this paper. To ensure stability the time-step $\Delta t$ must satisfy the Courant-Friedrichs-Lewy (CFL) criteria []
...
\end{gather}
 ...
 with $0<Cr\le 1$ where $\lambda$ is the wave speed. For the Serre equations it has been demonstrated that the wave speed is bounded by the wave speed of the Shallow Water Wave equations.[]
...
From Figure \ref{fig:FDsolh} it can be seen that $\mathcal{G}$ accurately models the highly non-linear soliton problem reproducing the solitary wave with the right speed up to graphical accuracy. Figure \ref{fig:GRsolhz} demonstrates serious problems with using $\mathcal{E}$ to numerically approximate the Serre equations. For $\mathcal{E}$ we have very similar results to those of the first order FDVM presented in [], however now instead of being overly diffusive our scheme is dissipative thus the height of the soliton is raised instead of decreased, increasing its celerity. To demonstrate that in fact $\mathcal{E}$ is consistent two measures are used. One measures the relative distance of the numerical results in both $h$ and $u$ from the analytic solution and is defined for a general quantity $q$ and an approximation to it $q^*$ at $n$ values like so

\begin{gather}
L_1 = \dfrac{\sum_{i = 0}^{n} \left| q_i - q^*_i\right|}{\sum_{i = 0}^{n} \left| q_i\right|}.
\end{gather}
The other measures how well the scheme conserves the Hamiltonian of the Serre equations. 
...
\begin{figure}
\centering
\subfigure[][]{\label{fig:FDsolh}\includegraphics[width=7cm]{./results/soliton/ex/FDcent/12/plotsolh-figure0.pdf} \includegraphics[width=7cm]{./results/soliton/zex/FDcent/12/plotsolh-figure0.pdf}}
\subfigure[][]{\label{fig:GRsolhz}\includegraphics[width=7cm]{./results/soliton/ex/grim/12/plotsolh-figure0.pdf} \includegraphics[width=7cm]{./results/soliton/zex/grim/12/plotsolh-figure0.pdf}}
\caption{Water profile for the soliton problem \eqref{eq:sol} for $\mathcal{G}$ (a) and $\mathcal{E}$ (b) when $\Delta x = 10/2^{12}$ with the initial conditions ($-$, black), analytic solution ($-$, blue) and numerical result ($\bullet$, red).}
\label{fig:FDMsolexp}
\end{figure}
\begin{figure}
\centering
\subfigure[][]{\label{fig:FDo2normL1}\includegraphics[width=7cm]{./results/soliton/norm/FDcent/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:FDo2Enorm}\includegraphics[width=7cm]{./results/soliton/norm/FDcent/E.pdf}}
\subfigure[][]{\label{fig:grimo2normL1}\includegraphics[width=7cm]{./results/soliton/norm/grim/sto1-figure0.pdf}}
\subfigure[][]{\label{fig:grimo2Enorm}\includegraphics[width=7cm]{./results/soliton/norm/grim/E.pdf}}
\caption{On the left $L_1$ errors for $h$ ($\triangle$, red) and $u$ ($\square$, blue) and on the right the relative error for the Hamiltonian ($\circ$, blue). With $\mathcal{G}$ on top and $\mathcal{E}$ on the bottom.}
...
%
From Figure \ref{fig:FDMsolnorm} it can be seen that both methods are convergent under $L_1$, but only $\mathcal{G}$ is second-order.[Explain $\mathcal{E}$ first-order]. The $L_1$ norm demonstrates that our methods converge to the analytic solution for the soliton problem, validating them in the smooth case. The $H_1$ value measures how well the method conserves the Hamiltonian, we see that these methods conserve the Hamiltonian well and converge to the correct value of $0$ for $H_1$. In [] it is observed that the Hamiltonian order of convergence increases with the FDVM, but this is not the case here. The cause of this is that finite difference methods are not necessarily conservative in their variables. This results in poor conservation of $h$ and $u$ which causes poor conservation of $\mathcal{H}$, although as $\Delta x$ approaches $0$ our method better approximates the PDE and so we better approximate this conservation of $\mathcal{H}$. 

These results demonstrate that these methods solve the Serre equations, although we do not recommend either especially $\mathcal{E}$ which has a suboptimal order of accuracy. These methods will be used to verify that the results presented below are not the result of the reformulation or something introduced by inappropriate limiting. 
...
For the results in Figure \ref{fig:o3a1dxlimflatexp} the Hamiltonian for the initial conditions is $10335.816$(units) the Hamiltonian was numerically calculated to be $10335.9175485$ (units) for the final time of the third-order method with $\Delta x = 10/2^{10}$. This gives a relative error of $9.82491350992 \times 10^{-6}$ for the conservation of the Hamiltonian. This low relative error suggests that our numerical method is solving the equations appropriately validating our results. 
...
\caption{Numerical results of $\mathcal{V}_3$  at $t= 50s$ for the smooth dam break problem with $\alpha = 0.025$ for $\Delta x = 10/2^{10}$ (blue), $\Delta x = 10/2^9$ (green), $\Delta x = 10/2^8$ (red), $\Delta x = 10/2^7$ (cyan), $\Delta x = 10/2^6$ (magenta), $\Delta x = 10/2^5$ (yellow), $\Delta x = 10/2^4$ (black).}
\label{fig:o3a1dxlimflatexp}
\end{figure}

\begin{figure}
\centering
\subfigure[][]{\label{fig:o1a1dxlimH}\includegraphics[width=7cm]{./results/Hcon/o1/betan0.025/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o2a1dxlimH}\includegraphics[width=7cm]{./results/Hcon/o2/betan0.025/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o3a1dxlimH}\includegraphics[width=7cm]{./results/Hcon/o3/betan0.025/sto2-figure0.pdf}}
%\subfigure[][]{\label{fig:o3a1dxlimz1}\includegraphics[width=7cm]{./results/FVMpics/1diffmdx/o3/1/nb3ne10/1-figure0.pdf}}
\caption{$H_1$ for $\mathcal{V}_1$ (a), $\mathcal{V}_2$ (b) and $\mathcal{V}_3$ (d) with $\alpha = 0.025$.}
%\label{fig:o3a1dxlimflatexp}
\end{figure}
...
\caption{Numerical results of $\mathcal{V}_3$  at $t= 50s$ for the smooth dam break problem with $\alpha = 0.5$ for $\Delta x = 10/2^{10}$ (blue), $\Delta x = 10/2^9$ (green), $\Delta x = 10/2^8$ (red), $\Delta x = 10/2^7$ (cyan), $\Delta x = 10/2^6$ (magenta), $\Delta x = 10/2^5$ (yellow), $\Delta x = 10/2^{4}$ (black).}
\label{fig:o3a6dxlimflatexp}
\end{figure}

\begin{figure}
\centering
\subfigure[][]{\label{fig:o1a6dxlimH}\includegraphics[width=7cm]{./results/Hcon/o1/betan0.5/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o2a6dxlimH}\includegraphics[width=7cm]{./results/Hcon/o2/betan0.5/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o3a6dxlimH}\includegraphics[width=7cm]{./results/Hcon/o3/betan0.5/sto2-figure0.pdf}}
%\subfigure[][]{\label{fig:o3a1dxlimz1}\includegraphics[width=7cm]{./results/FVMpics/1diffmdx/o3/1/nb3ne10/1-figure0.pdf}}
\caption{$H_1$ for $\mathcal{V}_1$ (a), $\mathcal{V}_2$ (b) and $\mathcal{V}_3$ (d) with $\alpha = 0.5$.}
%\label{fig:o3a1dxlimflatexp}
\end{figure}
...
\caption{Numerical results of $\mathcal{V}_3$  at $t= 50s$ for the smooth dam break problem with $\alpha = 2.5$ for $\Delta x = 10/2^{10}$ (blue), $\Delta x = 10/2^9$ (green), $\Delta x = 10/2^8$ (red), $\Delta x = 10/2^7$ (cyan), $\Delta x = 10/2^6$ (magenta), $\Delta x = 10/2^5$ (yellow), $\Delta x = 10/2^{4}$ (black).}
\label{fig:o3a9dxlimcdexp}
\end{figure}

\begin{figure}
\centering
\subfigure[][]{\label{fig:o1a9dxlimH}\includegraphics[width=7cm]{./results/Hcon/o1/betan2.5/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o2a9dxlimH}\includegraphics[width=7cm]{./results/Hcon/o2/betan2.5/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o3a9dxlimH}\includegraphics[width=7cm]{./results/Hcon/o3/betan2.5/sto2-figure0.pdf}}
%\subfigure[][]{\label{fig:o3a1dxlimz1}\includegraphics[width=7cm]{./results/FVMpics/1diffmdx/o3/1/nb3ne10/1-figure0.pdf}}
\caption{$H_1$ for $\mathcal{V}_1$ (a), $\mathcal{V}_2$ (b) and $\mathcal{V}_3$ (d) with $\alpha = 2.5$.}
%\label{fig:o3a1dxlimflatexp}
\end{figure}
...
\caption{Numerical results of $\mathcal{V}_3$  at $t= 50s$ for the smooth dam break problem with $\alpha = 1000.0$ for $\Delta x = 10/2^{10}$ (blue), $\Delta x = 10/2^9$ (green), $\Delta x = 10/2^8$ (red), $\Delta x = 10/2^7$ (cyan), $\Delta x = 10/2^6$ (magenta), $\Delta x = 10/2^7$ (yellow), $\Delta x = 10/2^{8}$ (black)}
\label{fig:o3a20dxlimcdexp}
\end{figure}

\begin{figure}
\centering
\subfigure[][]{\label{fig:o1a20dxlimH}\includegraphics[width=7cm]{./results/Hcon/o1/betan1000.0/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o2a20dxlimH}\includegraphics[width=7cm]{./results/Hcon/o2/betan1000.0/sto2-figure0.pdf}}
\subfigure[][]{\label{fig:o3a20dxlimH}\includegraphics[width=7cm]{./results/Hcon/o3/betan1000.0/sto2-figure0.pdf}}
%\subfigure[][]{\label{fig:o3a1dxlimz1}\includegraphics[width=7cm]{./results/FVMpics/1diffmdx/o3/1/nb3ne10/1-figure0.pdf}}
\caption{$H_1$ for $\mathcal{V}_1$ (a), $\mathcal{V}_2$ (b) and $\mathcal{V}_3$ (d) with $\alpha = 1000$.}
%\label{fig:o3a1dxlimflatexp}
\end{figure}
...
\begin{figure}
\centering
\subfigure[][]{\label{fig:FDa20dxlim}\includegraphics[width=7cm]{./results/FDpics/noise/h-figure0.pdf}}
\caption{Smooth dam break problem for $\mathcal{G}$ with $\alpha = 5.0$ for $\Delta x = 10/2^{5}$ (black)}
\label{fig:FDa20dxlimdexp}
\end{figure}
