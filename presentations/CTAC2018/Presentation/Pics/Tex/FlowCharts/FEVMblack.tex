...
\fontfamily{cmss}
 ...
 \node [right of= MM, node distance=3cm](CP) {$\hat{u}^n_j$};
 \node [right of= CP, node distance=4cm](CC) {$F^{n}_{j\pm 1/2}, S^n_j$};
 \path [line] (PP) -> (MM) node[pos=0.5,above,sloped] {Reconstruction};
 \path [line] (MM) -> (CP) node[pos=0.5,above,sloped, align=center] {Calculate \\ Velocity};
 \path [line] (CP) -> (CC) node[pos=0.5,above,sloped, align=center] {Flux \\ Approximation};
