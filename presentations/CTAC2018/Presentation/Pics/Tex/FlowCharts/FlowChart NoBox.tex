...
\fontfamily{cmss}
 ...
 \node [right of= MM, node distance=4cm](CP) {$\hat{u}^n_j$};
 \path [line] (PP) -> (MM) node[pos=0.5,above,sloped] {Reconstruction};
 \path [line] (MM) -> (CP) node[pos=0.5,above,sloped] {Finite Element};
 \path [line] (CP) -- ($ (CP) + (0,-1) $) --  ($ (PP) + (0,-1) $) ->  (PP) ;
 \node [below of=MM, node distance=1.3cm] {Finite Volume};
