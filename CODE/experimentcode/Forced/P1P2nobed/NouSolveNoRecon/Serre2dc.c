...
-36.0*(exp(0.5*t)*sin(2*x) + 10.0)**3*exp(1.4*t)*sin(3*x)*cos(3*x) - 36.0*(exp(0.5*t)*sin(2*x) + 10.0)**2*exp(1.9*t)*sin(3*x)**2*cos(2*x) + 19.62*(exp(0.5*t)*sin(2*x) + 10.0)*exp(0.5*t)*cos(2*x) - 3*(3*(exp(0.5*t)*sin(2*x) + 10.0)**3*exp(0.7*t)*cos(3*x) + 6*(exp(0.5*t)*sin(2*x) + 10.0)**2*exp(1.2*t)*sin(3*x)*cos(2*x) + (exp(0.5*t)*sin(2*x) + 10.0)*exp(0.7*t)*cos(3*x))*exp(0.7*t)*sin(3*x) + (-9*(exp(0.5*t)*sin(2*x) + 10.0)**3*exp(0.7*t)*sin(3*x) - 12*(exp(0.5*t)*sin(2*x) + 10.0)**2*exp(1.2*t)*sin(2*x)*sin(3*x) + 36*(exp(0.5*t)*sin(2*x) + 10.0)**2*exp(1.2*t)*cos(2*x)*cos(3*x) - 3*(exp(0.5*t)*sin(2*x) + 10.0)*exp(0.7*t)*sin(3*x) + 24*(exp(0.5*t)*sin(2*x) + 10.0)*exp(1.7*t)*sin(3*x)*cos(2*x)**2 + 2*exp(1.2*t)*cos(2*x)*cos(3*x))*exp(0.7*t)*cos(3*x)
...
void evolve(double *Ghbc, double *hhbc, double *ubc, int nGhhbc, int nubc, int nGhBC, int unBC, double g, double dx, double dt, int n, double theta, double *newG, double *newh, double *x, double t)
{
   ...
   // printf("%d | %f | %f | %f \n",i,ubc[2*i + unBC - 1],ubc[2*i + unBC],ubc[2*i + unBC + 1]);
...
void evolvewrapperconsistenttimeForced(double *G, double *h,double *hMbeg , double *hMend,double *GMbeg , double *GMend,double *uMbeg , double *uMend,double *hMbeg1 , double *hMend1,double *GMbeg1 , double *GMend1,double *uMbeg1, double *uMend1, double g, double dx, double dt, int n, int nGhBC, int unBC, int nGhhbc, int nubc, double theta, double *hhbc,double *Ghbc,double *ubc, double *Gp, double *hp, double *Gpp, double *hpp, double *x, double t, double *ubc1, double *ubcp1)
{
    ...
    evolve(Ghbc, hhbc, ubc1,nGhhbc,nubc,nGhBC,unBC,g, dx,dt,n, theta ,Gp, hp,x,t);
    //printf("1 | %f | %f \n",t,dt);
    ...
    evolve(Ghbc, hhbc, ubcp1,nGhhbc,nubc,nGhBC,unBC,g, dx,dt,n, theta ,Gpp, hpp,x,t + dt);
    //printf("1 | %f | %f \n",t+dt,dt);
