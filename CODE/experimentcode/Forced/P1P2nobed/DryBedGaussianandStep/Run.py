...
def Forced(x,t,c,x0,a0,a1,a2,g,h0,l):
    n = len(x)
    h = zeros(n)
    u = zeros(n)
    G = zeros(n)
    print(x0 + c*t - l, x0 + c*t + l)
    for i in range(n):
        ...
        if(x[i] <= x0 + c*t - l):
            ...
            uxi = -a0*(-2*a1 - 2*c*t + 2*x[i])*exp(-(-a1 - c*t + x[i])**2/(2*a2))/(2*a2)
            hxi = 0
            uxxi = a0*(-1 + (a1 + c*t - x[i])**2/a2)*exp(-(a1 + c*t - x[i])**2/(2*a2))/a2
...
wdir = "../../../../../../data/raw/Forced/o2FEMP1P2/DryBedGandS/t2/"  
...
for j in range(15,25):        
    ...
    h,u,G = Forced(x,ct,c,x0,a0,a1,a2,g,h0,l1)
    ...
    while ct < endt:
        evolvewrapperconsistenttimeForced(G_c, h_c,hMbeg_c , hMend_c,GMbeg_c ,GMend_c,uMbeg_c,uMend_c,hMbeg_c , hMend_c,GMbeg_c ,GMend_c,uMbeg_c,uMend_c,g,dx, dt,n,GhnBC,unBC,nGhhbc,nubc,theta, hhbc_c,Ghbc_c,ubc_c,Gp_c,hp_c, Gpp_c,hpp_c, x_c, ct)        
        ct = ct + dt
    ...
    hF,uF,GF = Forced(x,ct,c,x0,a0,a1,a2,g,h0,l1)
