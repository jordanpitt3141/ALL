...
def Forced(x,t,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5):
    n = len(x)
    h = zeros(n)
    u = zeros(n)
    G = zeros(n)
    ...
    for i in range(n):
        h[i] = (a0*x[i]**2 + a1*x[i] + a2)*(a3*t**2 + a4*t + a5)
        u[i] = (b0*x[i]**2 + b1*x[i] + b2)*(b3*t**2 + b4*t + b5)
        
        uxi = (2*b0*x[i] + b1)*(b3*t**2 + b4*t + b5)
        hxi = (2*a0*x[i] + a1)*(a3*t**2 + a4*t + a5)
        uxxi = (2*b0)*(b3*t**2 + b4*t + b5)
...
wdir = "../../../../../data/raw/Forced/o2FEMP1P2/QuadraticsF1/"  
...
for j in range(35):        
    ...
    h,u,G = Forced(x,0,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)

    
    hMbeg,uMbeg,GMbeg = Forced(xhMbeg,ct,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)  
    hMend,uMend,GMend = Forced(xhMend,ct,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)
     
    hMbeg1,uMbeg1,GMbeg1 = Forced(xhMbeg,ct + dt,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)
    hMend1,uMend1,GMend1 = Forced(xhMend,ct + dt,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)
    ...
    hF,uF,GF = Forced(x,ct,a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5)
    ...
    for i in range(len(x)):
        xuxbc.append(x[i] - 0.5*dx)
        xuxbc.append(x[i] + 0.5*dx)
        
        uxbcA.append(-a3*exp(a4*ct)*sin(a3*(x[i] - 0.5*dx)))
        uxbcA.append(-a3*exp(a4*ct)*sin(a3*(x[i] + 0.5*dx)))
    ...
    hhbcA,uta,GhbcA = Forced(xhbc,ct,a0,a1,a2,a3,a4,g)    
    hta,ubcA,Gta = Forced(xubc,ct,a0,a1,a2,a3,a4,g) 
    

    uxnorm = norm(uxbcA - uxbcC,ord =2) / norm(uxbcA,ord=2)
    unorm = norm(ubcA - ubcC,ord =2) / norm(ubcA,ord=2)
    hnorm = norm(hhbcA - hhbcC,ord =2) / norm(hhbcA,ord=2)
    Gnorm = norm(GhbcA - GhbcC,ord =2) / norm(GhbcA,ord=2)
    """
