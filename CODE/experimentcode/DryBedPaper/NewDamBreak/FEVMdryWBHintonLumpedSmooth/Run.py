...
def SmoothDambreak(h0,h1,x0,x,diffuse):
    n = len(x)
    h = zeros(n)
    u = zeros(n)
    G = zeros(n)
...
wdirbase = "/home/thanksjoe/Documents/newPhD/data/2019/DryBedPaper/Dambreak/NewSmallTime/MassLumpingWetRegions/"  
#wdirbase = "/home/jp/Documents/PhD/project/data/DryBedPaper/SmoothDBConv/t1/"

if not os.path.exists(wdirbase):
    os.makedirs(wdirbase)

s = wdirbase +  "Conservation.txt"
with open(s,'a') as file2:
    writefile2 = csv.writer(file2, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    writefile2.writerow(['dx' ,'dt','time',"E error", "M error", "Momentum error", "G error" ])   
...
for j in range(6,21):
    ...
    h,u,G,b,w = SmoothDambreak(h0,h1,x0,x,diff)
    
    hMbeg,uMbeg,GMbeg,bta,wMbeg = SmoothDambreak(h0,h1,x0,xhuMbeg,diff)
    hMend ,uMend ,GMend ,bta,wMend = SmoothDambreak(h0,h1,x0,xhuMend,diff)
    
    hta,uta,Gta,bMbeg,wta = SmoothDambreak(h0,h1,x0,xbMbeg,diff)
    hta,uta,Gta,bMend,wta = SmoothDambreak(h0,h1,x0,xbMend,diff)
    ...
    Pncorr = -0.5*g*(t-startt)*(h0**2 - h1**2)
    Gncorr = -0.5*g*(t-startt)*(h0**2 - h1**2)
    
    Eerr = abs(En- Eni)/ abs(Eni)
    Perr = abs(Pn- Pni - Pncorr)/ abs(Pni + Pncorr)
    Gerr = abs(Gn- Gni - Gncorr)/ abs(Gni + Gncorr)
    ...
    with open(s,'w') as file2:
        writefile2 = csv.writer(file2, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writefile2.writerow(['dx' ,'dt','time',"Energy", "Mass", "Momentum", "G","Energy0", "Mass0", "Momentum0", "G0", "Momentum Corr" , "G Corr"   ])   
        writefile2.writerow([str(dx),str(dt),str(t),str(En),str(Mn),str(Pn),str(Gn),str(Eni),str(Mni),str(Pni),str(Gni), str(Pncorr), str(Gncorr) ]) 


    s = wdirbase +  "Conservation.txt"
    with open(s,'a') as file2:
        writefile2 = csv.writer(file2, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writefile2.writerow([str(dx),str(dt),str(Eerr),str(Merr),str(Perr),str(Gerr)]) 
