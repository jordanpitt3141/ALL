...
void getufromG(double *hbc, double *Gbc, double *bbc, double *uMbeg, double *uMend, double dx , int n, int m, double *ubc)
{
    ...
    double Suhinit, Duhinit;
    double hjphm1, hjmhp1;
    ...
    hjphm1=  hbc[3*(i) + 2]*(hbc[3*(i) + 2] +hbase)/ (hbc[3*(i) + 2] + htol); 
    hjmhp1=  hbc[3*(i)]*(hbc[3*(i)] +hbase)/ (hbc[3*(i)] + htol); 
    hjphm=  hbc[3*(i) + 2]; 
    hjmhp=  hbc[3*(i)]; 
   ...
   // G integral (RHS)
    ...
    //uh integral
    uhintia11 = (1.0 *i60)*dx*(7*hjmhp1 + hjphm1);
    uhintia12 = (1.0 *i60)*dx*(4*hjmhp1 );
    uhintia13 = (1.0 *i60)*dx*(-hjmhp1 - hjphm1);
    
    uhintia21 = (1.0 *i60)*dx*(4*hjmhp1);
    uhintia22 = (1.0 *i60)*dx*(16*hjmhp1 + 16*hjphm1);
    uhintia23 = (1.0 *i60)*dx*(4*hjphm1);
    
    uhintia31 = (1.0 *i60)*dx*(-hjmhp1 - hjphm1);
    uhintia32 = (1.0 *i60)*dx*(4*hjphm1);
    uhintia33 = (1.0 *i60)*dx*(hjmhp1 + 7*hjphm1);
    
    Suhinit =  uhintia11 + uhintia12 + uhintia13 + uhintia21 + uhintia22 + uhintia23 + uhintia31 + uhintia32 + uhintia33;
    Duhinit =  uhintia11 + uhintia22 + uhintia33;
    
    uhintia11 = (Suhinit/Duhinit)*(1.0 *i60)*dx*(7*hjmhp1 + hjphm1);
    uhintia12 = 0;
    uhintia13 = 0;
    
    uhintia21 = 0;
    uhintia22 = (Suhinit/Duhinit)*(1.0 *i60)*dx*(16*hjmhp1 + 16*hjphm1);
    uhintia23 = 0;
    
    uhintia31 = 0;
    uhintia32 = 0;
    uhintia33 = (Suhinit/Duhinit)*(1.0 *i60)*dx*(hjmhp1 + 7*hjphm1);
...
//include BCs
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, double *FjhH, double *FjhG, double *SjG)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    for(i = 0;i < n;i++)
    {
        ...
        FjhH[i + 1] = foh;
        FjhG[i + 1] = foG;
        SjG[i] = dx*sourcec + sourcel + sourcer;
...
void Fluxsplit(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG)
{
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        if(RegIndx[i*m1 +  3] == 1)
        {
           ...
           // printf("%d | %d | %d | %d | %d  \n \n",hIbeg,uIbeg,bIbeg,FIbeg,SIbeg   );
            ...
            evolveForce(hbc+ hIbeg,Gbc+ hIbeg,wbc+ hIbeg,bbc + bIbeg,ubc+ uIbeg,g,dx,dt,RegIndx[i*m1 +  2],hnBC,hnbc,bnBC,bnbc,unBC,unbc,FjhH + FIbeg,FjhG + FIbeg ,SjG + SIbeg);
        }
    }
...
void FluxWrap(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *hp, double *Gp)
{
    double *FjhH = malloc((n+1)*sizeof(double));
    double *FjhG = malloc((n+1)*sizeof(double));
    double *SjG = malloc((n)*sizeof(double));
    
    Fluxsplit(hbc,Gbc,wbc,bbc,ubc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegIndx,m1,RegIndxLen,FjhH,FjhG,SjG);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc,double theta,double dx, double dt, double g)
{
    double idx = 1.0 /dx;
    ...
    FluxWrap(hbc,Gbc,wbc,bbc,ubc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegInd,m1 ,RegIndLen,hp,Gp);
    ...
    FluxWrap(hpbc,Gpbc,wpbc,bbc,upbc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegIndp,m1 ,RegIndLen,hpp,Gpp);
