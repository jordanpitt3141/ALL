...
//include BCs
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, double *FjhH, double *FjhG, double *SjG,double theta)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    double himhp,bedai,bedbi,bedci,bedaip1,bedbip1,bedcip1,beddi,beddip1,duim,duib,duif,duip1m,duip1b,duip1f,dui,duip1;
    ...
    duib = (ubc[2*i + unBC] - ubc[2*i + unBC - 1]);
    duim = 0.5*(ubc[2*i + unBC + 1] - ubc[2*i + unBC - 1]);
    duif = (ubc[2*i + unBC + 1] - ubc[2*i + unBC]);

    dui = minmod(theta*duib,duim,theta*duif);


    duip1b = (ubc[2*(i+1) + unBC] - ubc[2*(i+1) + unBC - 1]);
    duip1m = 0.5*(ubc[2*(i+1) + unBC + 1] - ubc[2*(i+1) + unBC - 1]);
    duip1f = (ubc[2*(i+1) + unBC + 1] - ubc[2*(i+1) + unBC]);
...
void Fluxsplit(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG, double theta)
{
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        if(RegIndx[i*m1 +  3] == 1)
        {
           ...
           // printf("%d | %d | %d | %d | %d  \n \n",hIbeg,uIbeg,bIbeg,FIbeg,SIbeg   );
            ...
            evolveForce(hbc+ hIbeg,Gbc+ hIbeg,wbc+ hIbeg,bbc + bIbeg,ubc+ uIbeg,g,dx,dt,RegIndx[i*m1 +  2],hnBC,hnbc,bnBC,bnbc,unBC,unbc,FjhH + FIbeg,FjhG + FIbeg ,SjG + SIbeg,theta);
        }
    }
...
void FluxWrap(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *hp, double *Gp,double theta)
{
    ...
    Fluxsplit(hbc,Gbc,wbc,bbc,ubc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegIndx,m1,RegIndxLen,FjhH,FjhG,SjG,theta);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc,double theta,double dx, double dt, double g, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    FluxWrap(hbc,Gbc,wbc,bbc,ubc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegInd,m1 ,RegIndLen,hp,Gp,theta);
    ...
    FluxWrap(hpbc,Gpbc,wpbc,bbc,upbc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,RegIndp,m1 ,RegIndLen,hpp,Gpp,theta);
