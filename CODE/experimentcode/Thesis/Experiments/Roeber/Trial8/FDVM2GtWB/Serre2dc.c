...
void uSolveandRecon(double *h, double *G, double *b,double *hMbeg, double *hMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,int n, int hnBC , int hnbc, int unBC , int unbc,double theta,double dx, double dt, double g, double *ubc, double *dubc,int *RegIndex ,int m,int RegLen)
{
   ...
   // printf("u solve BC : %f | %f | %f | %f | %f | %f \n",uMbeg[unBC-2],uMend[1],hMbeg[hnBC-2],hMend[1],bMbeg[hnBC-2],bMend[1]);
    ...
    for(i =0;i<RegLen;i++)
    {
        ...
        if(RegIndex[i*m +  3] == 1)
        {
           for(j=RegIndex[i*m]; j < RegIndex[i*m +  1] ; j++)
           {
               ...
               /* if(j < 3)
                {
		            printf("u %d : %f | %f | %f \n",j,ubc[unBC + 2*(j) - 1] ,ubc[unBC + 2*(j)] ,ubc[unBC + 2*(j) + 1] );
		    		//printf("du %d : %f | %f | %f \n",i,dubc[unBC + 2*(j) - 1] ,dubc[unBC + 2*(j)] ,dubc[unBC + 2*(j) + 1]);
		    	}*/
           }
        }
...
void Stagehbed(int n, double *h, double *b, double *w, int *RegionIndex, int m, int RegIndLen)
{
    ...
    {
        //printf("Reg %d : %d | %d | %d | %d : %d \n",i,RegionIndex[i*m],RegionIndex[i*m+1],RegionIndex[i*m+2],RegionIndex[i*m+3],RegionIndex[i*m+4]);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double *dbMbeg, double *dbMend,double *ddbCbeg, double *ddbCend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,double *hMbegdt, double *hMenddt,double *GMbegdt, double *GMenddt,double *wMbegdt, double *wMenddt,double *uMbegdt, double *uMenddt, double *duMbegdt, double *duMenddt,int n, int hnBC , int hnbc, int unBC , int unbc, int CnBC, int Cnbc,double theta,double dx, double dt, double g)
{
    ...
    uSolveandRecon(h,G,b,hMbeg,hMend,bMbeg,bMend,uMbeg,uMend,duMbeg,duMend,n,hnBC,hnbc, unBC, unbc,theta,dx,dt,g,uFDbc,duFDbc,RegInd,m1,RegIndLen);
    ...
    ReconWrap(hp,Gp,b,hMbegdt,hMenddt,GMbegdt,GMenddt,wMbegdt,wMenddt,dbMbeg,dbMend,ddbCbeg,ddbCend,uMbegdt,uMenddt,n,hnBC,hnbc,unBC,unbc,CnBC,Cnbc,theta,dx,dt,g,wp,hpbc,Gpbc,wpbc,bMbc,dbMbc,ddbCbc,RegIndp,m1,RegIndLen);

    uSolveandRecon(hp,Gp,b,hMbegdt,hMenddt,bMbeg,bMend,uMbegdt,uMenddt,duMbegdt,duMenddt,n,hnBC,hnbc, unBC, unbc,theta,dx,dt,g,upFDbc,dupFDbc,RegIndp,m1,RegIndLen);
