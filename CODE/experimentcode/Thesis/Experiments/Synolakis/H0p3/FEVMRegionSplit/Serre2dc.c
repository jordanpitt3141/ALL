...
void dealloc22Int(int** array,int n)
{
    for (int i = 0; i < n; i++)  
    {
       free(array[i]);  
...
void writeto2memINT(int **x, int i, int j , double f)
{
    x[i][j] = f;

}

int readfrom2DmemINT(int **x,int i,int j)
{
    return x[i][j];
}
//HMMM memory

int **append(int **A, int n, int m, int *B, int ini)
{
    int **NA;
    int i,j;

    NA = malloc22In(n+1,m+1 );
...
int **RegSplit(double *h,int n)
{
    ...
    for (i = 0; i<n;i++)
    {
        ...
        if(regvali != Cregval)
        {
            ...
            arr[2] = (Cei - Csi);
            arr[3] = Cregval;
            Reg = append(Reg, ini, 4, arr,ini+1);
            Csi = i ;
            Cregval =regvali ;
            ini = ini + 1;
        }
...
void getufromGsplit(double *h, double *G, double *bed, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double theta, double dx , int n, int m, int nGhBC,int unBC,int nbBC, int nGhbc, int nubc, int nbbc, double *u, double *hhbc,double *Ghbc, double *whbc, double *bedhbc,int **RegInd)
{
   double idx = 1.0 / dx;
   int RegIndLen = RegInd[0][4];
   int i,j,hbegI,bbegI;
   ...
   int hIbeg,hIend,uIbeg,uIend,bIbeg,bIend;
    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i][3] != 1)
        {
           for(j=RegInd[i][0]; j <= RegInd[i][1] ; j++)
           {
                u[(unBC-1) + 2*j] = 0;
                u[(unBC-1) + 2*j +1] = 0;

                hhbc[nGhBC + 3*j] = htol;
                hhbc[nGhBC + 3*j + 1] = htol;
                hhbc[nGhBC + 3*j + 2] = htol;
                ...
                if(j == 1) 
                {
                    ...
                    beddi = -bedbegmiddle*i6 + 2*i3*bed[i-1] + 2*i3*bed[j+1] - bed[j+2]*i6;
                }
                else if(j == n-2)
                {
                    bedendmiddle = -bMend[0]*i16 + 9*bMend[1]*i16 + 9*bMend[2]*i16 - bMend[3]*i16;
    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i][3] == 1)
        {
            hIbeg = nGhBC + 3*RegInd[i][0];
            hIend = nGhBC + 3*(RegInd[i][1]) + 3;

            uIbeg = (unBC-1) + 2*RegInd[i][0];
            uIend = (unBC-1) + 2*(RegInd[i][1]) + 2;
            
            bIbeg = nbBC + 4*RegInd[i][0];
            bIend = nbBC + 4*(RegInd[i][1]) + 4;
            // give h + offset for h
            // use hhbc to get hbeg
            // w

            getufromG(h + RegInd[i][0] ,G + RegInd[i][0] ,bed + RegInd[i][0],hhbc+hIbeg -nGhBC,hhbc+hIend,Ghbc+hIbeg -nGhBC,Ghbc+hIend,u+uIbeg -unBC,u+uIend,whbc+hIbeg -nGhBC,whbc+hIend,bedhbc + bIbeg - nbBC,bedhbc + bIend,theta,dx , (RegInd[i][2] +1) , 2*(RegInd[i][2]+1) + 1,nGhBC,unBC,nbBC,nGhbc,nubc,nbbc,u + uIbeg,hhbc+ hIbeg ,Ghbc+hIbeg ,whbc+hIbeg ,bedhbc+bIbeg );
        }
    }
...
//include BCs
void evolveForce(double *Ghbc, double *hhbc, double *whbc, double *ubc,double *bedhbc, double g, double dx, double dt, int n, double theta, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7,double a8, double a9 ,double *hFluxes,double *hSource,double *GFluxes,double *GSource)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    uai =2*idx*idx*(ubc[2*i  - 1] - 2*ubc[2*i ] + ubc[2*i + 1]);
    ubi =idx*(-ubc[2*i - 1]+ ubc[2*i + 1]);

    bedai = 0.5*9*idx*idx*idx*(-bedhbc[4*i]+ 3*bedhbc[4*i+1] - 3*bedhbc[4*i+2] + bedhbc[4*i +3]);
    bedbi = 0.25*9*idx*idx*(bedhbc[4*i ] - bedhbc[4*i+1] - bedhbc[4*i+2] + bedhbc[4*i +3]);
    bedci = i8*idx*(bedhbc[4*i] - 27*bedhbc[4*i +1] + 27*bedhbc[4*i +2]- bedhbc[4*i+3]);
    //beddi = -bedhbc[4*i + bnBC]*i16 + 9*bedhbc[4*i + bnBC+1]*i16 + 9*bedhbc[4*i + bnBC+2]*i16 - bedhbc[4*i + bnBC]*i16;

    uaip1 =2*idx*idx*(ubc[2*(i+1) - 1] - 2*ubc[2*(i+1)] + ubc[2*(i+1) + 1]);
    ubip1 =idx*(-ubc[2*(i+1) - 1]+ ubc[2*(i+1) + 1]);

    bedaip1 = 0.5*9*idx*idx*idx*(-bedhbc[4*(i+1)]+ 3*bedhbc[4*(i+1)+1] - 3*bedhbc[4*(i+1) +2] + bedhbc[4*(i+1)  +3]);
    bedbip1 = 0.25*9*idx*idx*(bedhbc[4*(i+1) ] - bedhbc[4*(i+1)+1] - bedhbc[4*(i+1) +2] + bedhbc[4*(i+1) +3]);
    bedcip1 = i8*idx*(bedhbc[4*(i+1)] - 27*bedhbc[4*(i+1)+1] + 27*bedhbc[4*(i+1)+2]- bedhbc[4*(i+1) +3]);
    ...
    //printf("%d | %f | %f | %f" ,hhbc[3*i],hhbc[3*i + 1],hhbc[3*i + 2]);
    //printf("%d | %f | %f | %f" ,hhbc[3*i],hhbc[3*i + 1],hhbc[3*i + 2]);
    ...
    for(i = 0;i < n;i++)
    {
		...
		sourcer = g*0.5*(hihm*hihm - hhbc[3*(i)+2]*hhbc[3*(i) +2]);
		sourcec = -g*th*tbx -  0.5*th*th*tu*tux*tbxx + th*tu*tu*tbx*tbxx ;
		sourcel = g*0.5*(hhbc[3*(i)]*hhbc[3*(i)] - himhp*himhp);
        ...
        hSource[i] = 0;
        hFluxes[i + 1] =  foh;
        GSource[i] = dt*idx*(sourcer+sourcel + dx*sourcec);
        GFluxes[i +1] =  foG;
...
void evolveForceSplit(double *Ghbc, double *hhbc, double *whbc, double *ubc,double *bedhbc, int nGhhbc, int nubc, int nbhc, int nGhBC, int unBC, int bnBC, double g, double dx, double dt, int n, double theta, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7,double a8, double a9 ,double *hFlux,double *hSource,double *GFlux,double *GSource,int **RegInd)
{
   double idx = 1.0 / dx;
   int RegIndLen = RegInd[0][4];
   int i,j,hbegI,bbegI;
   ...
   int hIbeg,hIend,uIbeg,uIend,bIbeg,bIend;
    ...
    for (i =0;i<RegIndLen;i++)
    {
        ...
        else if(RegInd[i][3] == 1)
        {

            
            hIbeg = nGhBC + 3*RegInd[i][0];
            uIbeg = (unBC) + 2*RegInd[i][0];            
            bIbeg = bnBC + 4*RegInd[i][0];

            //printf("Flux Calc : %f | %f | %f | %f \n",RegInd[i][0],hIbeg,uIbeg,bIbeg);

            evolveForce(Ghbc + hIbeg , hhbc+ hIbeg,whbc + hIbeg, ubc + uIbeg, bedhbc + bIbeg,g, dx,dt,RegInd[i][2] + 1, theta,x+RegInd[i][0],t,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,hFlux +RegInd[i][0] ,hSource+RegInd[i][0],GFlux+RegInd[i][0],GSource+RegInd[i][0]);
        }
...
void evolvewrapForcing(double *G, double *h,double *bed,double *hMbeg , double *hMend,double *wMbeg , double *wMend,double *GMbeg , double *GMend,double *uMbeg , double *uMend, double *bMbeg, double *bMend,double *hMbeg1, double *hMend1,double *wMbeg1, double *wMend1,double *GMbeg1, double *GMend1,double *uMbeg1, double *uMend1, double g, double dx, double dt, int n, int nGhBC, int unBC,int bnBC, int nGhhbc, int nubc, int nbhc , double theta, double *hhbc, double *whbc,double *Ghbc,double *bedhbc,double *ubc, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double a8, double a9)
{
    ...
    double *hSource = malloc((n)*sizeof(double));
    double *hFlux  = malloc((n+1)*sizeof(double));
    double *GSource = malloc((n)*sizeof(double));
    double *GFlux  = malloc((n+1)*sizeof(double));
    int **Reg1,**Reg2;

    //Get wet/dry regions
    Reg1 = RegSplit(h,n);
    //Solve for u given wet/dry regions
    getufromGsplit(h,G,bed,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,wMbeg,wMend,bMbeg,bMend,theta,dx ,n,u_length ,nGhBC,unBC,bnBC,nGhhbc,nubc,nbhc,ubc,hhbc,Ghbc,whbc,bedhbc,Reg1);
    //Get fluxes and sources
    evolveForceSplit(Ghbc, hhbc,whbc, ubc, bedhbc,nGhhbc,nubc,nbhc,nGhBC,unBC,bnBC,g, dx,dt,n, theta,x,t,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,hFlux,hSource,GFlux,GSource,Reg1);
    int i;
    for(i=0;i<n;i++)
    {
        //printf("%d | %3.18f | %3.18f  | %3.18f \n",i, h[i-1],h[i],h[i+1]);   
        //printf("   | %3.18f       | %3.18f \n",i, hFlux[i],hFlux[i + 1]);        

        Gp[i] = G[i] - dt*idx*(GFlux[i + 1] - GFlux[i]) + GSource[i];
        hp[i] = h[i] - dt*idx*(hFlux[i + 1] - hFlux[i]) + hSource[i];
    ...
    Reg2 = RegSplit(hp,n);
    //Solve for u given wet/dry regions
    getufromGsplit(hp,Gp,bed,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,wMbeg,wMend,bMbeg,bMend,theta,dx ,n,u_length ,nGhBC,unBC,bnBC,nGhhbc,nubc,nbhc,ubc,hhbc,Ghbc,whbc,bedhbc,Reg2);
    //Get fluxes and sources
    evolveForceSplit(Ghbc, hhbc,whbc, ubc, bedhbc,nGhhbc,nubc,nbhc,nGhBC,unBC,bnBC,g, dx,dt,n, theta,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,hFlux,hSource,GFlux,GSource,Reg2);
    for(i=0;i<n;i++)
    {
        Gpp[i] = Gp[i] - dt*idx*(GFlux[i + 1] - GFlux[i]) + GSource[i];
        hpp[i] = hp[i] - dt*idx*(hFlux[i + 1] - hFlux[i]) + hSource[i];
