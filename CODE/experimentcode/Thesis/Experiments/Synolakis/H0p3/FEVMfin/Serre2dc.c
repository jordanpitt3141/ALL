...
double HankEnergyacrosscell(double *x,double *h,double *u, double *b,double g,int j,double dx)
{
    ...
    double fgpe = fgph*fgpu*fgpu + 2*g*fgph*(0.5*fgph + fgpb) + i3*fgph*fgph*fgph*fgpux*fgpux  + fgpu*fgpu*fgph*fgpbx - fgpu*fgph*fgph*fgpux*fgpbx;
    ...
    double sgpe = sgph*sgpu*sgpu + 2*g*sgph*(0.5*sgph + sgpb) + i3*sgph*sgph*sgph*sgpux*sgpux  + sgpu*sgpu*sgph*sgpbx - sgpu*sgph*sgph*sgpux*sgpbx;
    ...
    double tgpe = tgph*tgpu*tgpu + 2*g*tgph*(0.5*tgph + tgpb) + i3*tgph*tgph*tgph*tgpux*tgpux  + tgpu*tgpu*tgph*tgpbx - tgpu*tgph*tgph*tgpux*tgpbx;
...
void getufromG(double *hbc, double *Gbc, double *bbc, double *uMbeg, double *uMend, double dx , int n, int m, double *ubc)
{
    ...
    double Gjphm,Gjmhp,hjphm,hjmhp;
...
//include BCs
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, double *newG, double *newh)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    for(i = 0;i < n;i++)
    {
		...
		newh[i] = hbc[hnBC + 3*(i) + 1] - dt*idx*(foh - fih);
		newG[i] = Gbc[hnBC + 3*(i) + 1] - dt*idx*(foG - fiG) + dt*idx*(dx*sourcec + sourcer + sourcel);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc,double theta,double dx, double dt, double g)
{
    ...
    evolveForce(hbc,Gbc,wbc,bbc,ubc,g, dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,Gp, hp); 


    int *RegIndp = RegSplit(hp,n); 
    int RegIndLenp = RegIndp[m1-1];

    Stagehbed(n,hp,b,wp,RegIndp,m1,RegIndLenp);
    ...
    ReconZero(hpbc,n,hnBC,hnbc,RegIndp,m1,RegIndLenp);
    ReconZero(Gpbc,n,hnBC,hnbc,RegIndp,m1,RegIndLenp);

    getufromGsplit(hpbc, Gpbc, bbc,uMbeg,uMend,dx ,n,u_length ,hnBC,bnBC,hnbc,bnbc,unBC,unbc,RegIndp,m1,RegIndLenp,upbc);

    evolveForce(hpbc,Gpbc,wpbc,bbc,upbc,g, dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,Gpp, hpp); 
