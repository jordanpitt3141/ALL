...
wdir = "../../../../data/2018/raw/Thesis/Forced/Dry/FDVM/"
...
for ki in range(5,20):
    ...
    sx = -pi/2.0*10 -width/2.0
    ex = -pi/2.0*10 +width/2.0
    st = 0.0
    et = 1
    
    dx = width / (2.0)**(ki)
    l =  0.5 / (a6*exp(a7*et) + sqrt(g*(a0 + a1*exp(a5*et))))
    dt = l*dx
    ...
    if not os.path.exists(wdirji):
        os.makedirs(wdirji)
    s = wdirji  +  "outlast.txt"
    with open(s,'a') as file2:
        writefile2 = csv.writer(file2, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    
        writefile2.writerow(['dx' ,'dt','time',"cell midpoint" ,'h', 'G' , 'u(m/s)','bed', 'ht','ut','Gt' ])        
                   
        for ji in range(n):
            writefile2.writerow([str(dx),str(dt),str(t),str(x[ji]), str(hC[ji]) , str(GC[ji]) , str(uC[ji]),str(b[ji]),str(hA[ji]),str(uA[ji]),str(GA[ji])])
