...
void getufromGsplit(double *h, double *G, double *bed, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double theta, double dx , int n, int m, int nGhBC,int unBC,int nbBC, int nGhbc, int nubc, int nbbc, double *u, double *hhbc,double *Ghbc, double *whbc, double *bedhbc)
{
   ...
   //int RegIndLen = RegInd[m1-1];
    ...
    memcpy(u,uMbeg, (unBC)*sizeof(double));
    memcpy(u+nubc - (unBC),uMend,(unBC)*sizeof(double));
    ...
    getufromG(h ,G,bed,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,wMbeg,wMend,bMbeg,bMend,theta,dx , n , 2*(n) + 1,nGhBC,unBC,nbBC,nGhbc,nubc,nbbc,u + uIbeg,hhbc+ hIbeg ,Ghbc+hIbeg ,whbc+hIbeg ,bedhbc+bIbeg );
    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i*m1 +  3] == 1)
        {
            ...
            hIend = nGhBC + 3*(RegInd[i*m1 + 1]);

            uIbeg = (unBC-1) + 2*RegInd[i*m1];
            uIend = (unBC-1) + 2*(RegInd[i*m1 + 1]);
            
            bIbeg = nbBC + 4*RegInd[i*m1];
            bIend = nbBC + 4*(RegInd[i*m1 + 1]);
            // give h + offset for h
            // use hhbc to get hbeg
            // w

            getufromG(h + RegInd[i*m1] ,G + RegInd[i*m1] ,bed + RegInd[i*m1],hhbc+hIbeg -nGhBC,hhbc+hIend,Ghbc+hIbeg -nGhBC,Ghbc+hIend,u+uIbeg + 1 -unBC,u+uIend,whbc+hIbeg -nGhBC,whbc+hIend,bedhbc + bIbeg - nbBC,bedhbc + bIend,theta,dx , (RegInd[i*m1 + 2]) , 2*(RegInd[i*m1+2]) + 1,nGhBC,unBC,nbBC,nGhbc,nubc,nbbc,u + uIbeg,hhbc+ hIbeg ,Ghbc+hIbeg ,whbc+hIbeg ,bedhbc+bIbeg );
        }
    }
...
void evolvewrapForcingUANA(double *G, double *h,double *bed,double *hMbeg , double *hMend,double *wMbeg , double *wMend,double *GMbeg , double *GMend,double *uMbeg , double *uMend, double *bMbeg, double *bMend,double *hMbeg1, double *hMend1,double *wMbeg1, double *wMend1,double *GMbeg1, double *GMend1,double *uMbeg1, double *uMend1, double g, double dx, double dt, int n, int nGhBC, int unBC,int bnBC, int nGhhbc, int nubc, int nbhc , double theta, double *hhbc, double *whbc,double *Ghbc,double *bedhbc,double *ubc, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double a8, double a9, double *ubct, double *ubctpdt)
{
    double idx = 1.0 /dx;
    //n is number of cells
    int u_length = 2*n + 1;
    ...
    getufromGsplit(h,G,bed,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,wMbeg,wMend,bMbeg,bMend,theta,dx ,n,u_length ,nGhBC,unBC,bnBC,nGhhbc,nubc,nbhc,ubc,hhbc,Ghbc,whbc,bedhbc);
    //Get fluxes and sources
    evolveForce(Ghbc, hhbc,whbc, ubct, bedhbc,nGhhbc,nubc,nbhc,nGhBC,unBC,bnBC,g, dx,dt,n, theta ,Gp, hp,x,t,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9);

    //Solve for u given wet/dry regions
    getufromGsplit(hp,Gp,bed,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,wMbeg,wMend,bMbeg,bMend,theta,dx ,n,u_length ,nGhBC,unBC,bnBC,nGhhbc,nubc,nbhc,ubc,hhbc,Ghbc,whbc,bedhbc);
    //Get fluxes and sources
    evolveForce(Ghbc, hhbc,whbc, ubctpdt, bedhbc,nGhhbc,nubc,nbhc,nGhBC,unBC,bnBC,g, dx,dt,n, theta ,Gpp, hpp,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7,a8,a9); 
