    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i*m1 +  3] == 1)
        {
           ...
           for(j=RegInd[i*m1]; j < RegInd[i*m1 +  1] ; j++)
           {
                ...
                uEbc[(nEBC - 1) + 2*j] = uN[j - RegInd[i*m1]] ;
                uEbc[(nEBC - 1) + 2*j + 1] =  0.5*(uN[j - RegInd[i*m1]] +  uNp1) ;
...
void evolveDRY(double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *duEbc, double *uEbc, double *ddbCbc, int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, double dx, double dt, double g, double theta, double *hblank, double *Gblank)
{
    ...
    double hip1l,hir,Gip1l,Gir,wip1l,wir,bip1l,bir,ue,due,hil,bil;
    ...
    for(i =nCBC; i < n + nCBC;i++)
    {
		...
		hblank[i - nCBC] = hMbc[nMBC + 3*i -2] - dt*idx*(foh - fih);
		Gblank[i - nCBC] = GMbc[nMBC + 3*i -2] - dt*idx*(foG -fiG) + dt*idx*( dx*sourcec);
        //printf("%d: %f | %f | %f | %f \n",i - nCBC,GMbc[nMBC + 3*i -2], - dt*idx*(foG -fiG),sourcer, sourcel);
...
//UNFINISHED~~~
	...
	evolveDRY(hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,dx,dt,g,theta,hp,Gp);
	...
	evolveDRY(hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,dx,dt,g,theta,hpp,Gpp);
