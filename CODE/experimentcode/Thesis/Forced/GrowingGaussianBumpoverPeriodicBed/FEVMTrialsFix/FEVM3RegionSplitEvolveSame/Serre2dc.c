...
void getufromGsplit(double *h, double *G, double *bed, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double theta, double dx , int n, int m, int nGhBC,int unBC,int nbBC, int nGhbc, int nubc, int nbbc, double *u, double *hhbc,double *Ghbc, double *whbc, double *bedhbc)
{
   ...
   double Gjm1,hjm1,bjm1,Gjp1,hjp1,bjp1,wi,dGib,dGim,dGif,dhib,dhim,dhif,wip1,wim1,dwim,dwif,dwib,dhi,dGi,dwi,hjphm,hjmhp,Gjphm,Gjmhp,wjphm,wjmhp,hjt,hjpht;
        ...
        if(RegInd[i*m1 +  3] == 2)
        {
           for(j=RegInd[i*m1]; j < RegInd[i*m1 +  1] ; j++)
           {
                ...
                if(j == 0)
                {
                    Gjm1 = GMbeg[1];
                    hjm1 = hMbeg[1];
                    bjm1 =  -bMbeg[0]*i16 + 9*bMbeg[1]*i16 + 9*bMbeg[2]*i16 - bMbeg[3]*i16;
                }
                else
                {
                ...
                if(j == n-1)
                {
                    Gjp1 = GMend[1];
                    hjp1 = hMend[1];
                    bjp1 = -bMend[0]*i16 + 9*bMend[1]*i16 + 9*bMend[2]*i16 - bMend[3]*i16;
                }
                else
                {
                ...
                hjt = h[j]*((h[j] + hbase)/ h[j]);
                hjpht = (0.5*(h[j] + hjp1))*(((0.5*(h[j] + hjp1)) + hbase)/ (0.5*(h[j] + hjp1)));

                u[(unBC-1) + 2*j] = G[j] / hjt;
                u[(unBC-1) + 2*j +1] = (0.5*(G[j] + Gjp1)) / hjpht;

                hhbc[nGhBC + 3*j] = hjphm;
                hhbc[nGhBC + 3*j + 1] = h[j];
                hhbc[nGhBC + 3*j + 2] = hjmhp;

                Ghbc[nGhBC + 3*j] = Gjphm;
                Ghbc[nGhBC + 3*j + 1] = G[j];
                Ghbc[nGhBC + 3*j + 2] = Gjmhp;

                whbc[nGhBC + 3*j] = wjphm;
                whbc[nGhBC + 3*j + 1] = wi;
                whbc[nGhBC + 3*j + 2] = wjmhp;
