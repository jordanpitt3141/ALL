...
int ContWetCheck(double *h, int i, int RegTol1, double htol1)
{
    int out1 = 0;
    int j;
    for(j = i; j < i + RegTol1; j ++)
    {
        out1 = out1 + !(h[j] > htol1);
    }
...
int *RegSplit(double *h,int n)
{
    ...
    for (i = 0; i<n;i++)
    {
        ...
        if(regvali == 1 && Cregval == 0)
        {
            //check that next wet region is large enough

            if(i  + RegTol < n && ContWetCheck(h,i,RegTol,htol))
            {
                // new one
                Cei = i;
                arr[0] = Csi;
...
void Fluxsplit(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG)
{
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        if(RegIndx[i*m1 +  3] == 0)
        {
           ...
           SjG[RegIndx[i*m1]] = 0;
           SjG[RegIndx[i*m1 +  1]  - 1] = 0;


           for(j=RegIndx[i*m1] + 1; j < RegIndx[i*m1 +  1]  - 1; j++)
           {
                FjhH[j] = 0;
                FjhG[j] = 0;
                SjG[j] = 0;
...
void FluxEvo(double *hbc, double *Gbc,double g, double dx, double dt, int n, int hnBC, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *FjhH, double *FjhG, double *SjG, double *hp, double *Gp)
{
    double hS,GS;
    int i;
    double idx = 1.0 / dx;
    for (i = 0; i < n; i++)
    {
        ...
        hp[i] = hbc[hnBC + 3*(i) + 1] - dt*idx*(FjhH[i+1] - FjhH[i]) + dt*hS;
        Gp[i] = Gbc[hnBC + 3*(i) + 1] - dt*idx*(FjhG[i+1] - FjhG[i]) + dt*idx*(SjG[i]) + dt*GS;
...
void FluxWrap(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int *RegIndx, int m1 ,int RegIndxLen, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *hp, double *Gp)
{
    ...
    FluxEvo(hbc,Gbc,g,dx,dt,n,hnBC,x,t,a0,a1,a2,a3,a4,a5,a6,a7,FjhH,FjhG,SjG,hp,Gp);
    free(FjhH);
    free(FjhG);
    free(SjG);
