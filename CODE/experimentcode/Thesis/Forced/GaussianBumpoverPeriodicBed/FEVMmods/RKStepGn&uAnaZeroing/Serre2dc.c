...
double SourceG(double x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double g)
{
    ...
    return g*h*bx  + h*h*ux*u*bxx/2.0 - h*u*bx*bxx;
}

//include BCs
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bbc, double *ubc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, double *newG, double *newh, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    uer  = uFunc(x[0] - 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
    duer = uxFunc(x[0] - 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);


    hel = hbc[hnBC + 3*(i) + 2] ;
    Gel = Gbc[hnBC + 3*(i) + 2];
    dbel = 3*bedai*(0.5*dx)*(0.5*dx) + 2*bedbi*(0.5*dx) + bedci;
    uel  = uFunc(x[0] - 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
    duel = uxFunc(x[0] - 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
    ...
    for(i = 0;i < n;i++)
    {
        ...
        Ger = Gbc[hnBC + 3*(i+1)];
        dber = 3*bedaip1*(0.5*dx)*(0.5*dx) - 2*bedbip1*(0.5*dx) + bedcip1;
        uer  = uFunc(x[i] + 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
        duer = uxFunc(x[i] + 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);


        hel = hbc[hnBC + 3*(i) + 2] ;
        Gel = Gbc[hnBC + 3*(i) + 2];
        dbel = 3*bedai*(0.5*dx)*(0.5*dx) + 2*bedbi*(0.5*dx) + bedci;
        uel  = uFunc(x[i] + 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
        duel = uxFunc(x[i] + 0.5*dx,t,a0,a1,a2,a3,a4,a5,a6,a7,g);
        ...
        //centerted values
        th = hbc[hnBC + 3*(i) + 1];
		tu =  uFunc(x[i],t,a0,a1,a2,a3,a4,a5,a6,a7,g);
		tux = uxFunc(x[i],t,a0,a1,a2,a3,a4,a5,a6,a7,g);
		tbx = bedci;
		tbxx =2*bedbi;
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc,double theta,double dx, double dt, double g, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    double *Gppbc = malloc((hnbc)*sizeof(double));
    double *hppbc = malloc((hnbc)*sizeof(double));
    ...
    evolveForce(hbc,Gbc,wbc,bbc,ubc,g, dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,Gp, hp,x,t,a0,a1,a2,a3,a4,a5,a6,a7); 


    RegInd = RegSplit(hp,n); 
    m1 = 5;
    RegIndLen = RegInd[m1-1];

    Stagehbed(n,hp,b,wp,RegInd,m1,RegIndLen);

    //Solve for u given wet/dry regions
    ReconLin(hp, hMbeg, hMend,n,hnBC,hnbc,theta,hpbc);
    ReconLin(Gp, GMbeg, GMend,n,hnBC,hnbc,theta,Gpbc);
    ReconLin(wp, wMbeg, wMend,n,hnBC,hnbc,theta,wpbc);

    ReconZero(hpbc,n,hnBC,hnbc,RegInd,m1,RegIndLen);
    ReconZero(Gpbc,n,hnBC,hnbc,RegInd,m1,RegIndLen);

    getufromGsplit(hpbc, Gpbc, bbc,uMbeg,uMend,dx ,n,u_length ,hnBC,bnBC,hnbc,bnbc,unBC,unbc,RegInd,m1,RegIndLen,upbc);

    evolveForce(hpbc,Gpbc,wpbc,bbc,upbc,g, dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,Gpp, hpp,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7); 

    RegInd = RegSplit(hpp,n);
    RegIndLen = RegInd[m1-1];
    ReconLin(hpp, hMbeg, hMend,n,hnBC,hnbc,theta,hppbc);
    ReconLin(Gpp, GMbeg, GMend,n,hnBC,hnbc,theta,Gppbc);

    ReconZero(hppbc,n,hnBC,hnbc,RegInd,m1,RegIndLen);
    ReconZero(Gppbc,n,hnBC,hnbc,RegInd,m1,RegIndLen);

    int i;
    for(i=0;i<n;i++)
    {
        //G[i] =0.5*(G[i] + Gpp[i]);
        h[i] =0.5*(hbc[hnBC + 3*(i) + 1] + hppbc[hnBC + 3*(i) + 1]);
        G[i] = 0.5*( GFunc(x[i],t,a0,a1,a2,a3,a4,a5,a6,a7,g) + Gppbc[hnBC + 3*(i) + 1]);
