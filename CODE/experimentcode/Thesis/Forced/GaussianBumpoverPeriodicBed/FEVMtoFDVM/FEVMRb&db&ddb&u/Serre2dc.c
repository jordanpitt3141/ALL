...
void getufromGFD(double *h, double *G,double *bed, double u0, double u1, double h0, double h1, double b0, double b1,double dx, int n,double *u)
{
	...
	for(i = 1; i < n-1; i++)
	{
		...
		thx = 0.5*idx*(h[i+1]*( (h[i+1] + hbase) / (h[i+1] + htol)) - h[i-1]*( (h[i-1] + hbase) / (h[i-1] + htol)));
	...
	i = 0;
	th = h[i]*( (h[i] + hbase) / (h[i] + htol));
	thx = 0.5*idx*(h[i+1]*( (h[i+1] + hbase) / (h[i+1] + htol)) - h0*( (h0 + hbase) / (h0 + htol)));
	...
	th = h[i]*( (h[i] + hbase) / (h[i] + htol));
	thx = 0.5*idx*(h1*( (h1 + hbase) / (h1 + htol))- h[i-1]*( (h[i-1] + hbase) / (h[i-1] + htol)));
...
void uSolveandRecon(double *h, double *G, double *b,double *hMbeg, double *hMend,double *bMbeg, double *bMend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc,double theta,double dx, double dt, double g, double *ubc, double *dubc,int *RegIndex ,int m,int RegLen)
{
    int i,j;
    double ujm1,ujp1;
    double idx = 1.0 / dx;
...
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bbc, double *bMbc, double *dbMbc, double *ddbCbc, double *ubc , double *uFDbc,double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int CnBC, int Cnbc, double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x, double t, double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7)
{
    double idx = 1.0 / dx;  
	int i;
    ...
    */
    ...
    for(i = 0;i < n;i++)
    {
        ...
        */
        ...
        //printf("%d :  %e | %e | %e \n ",i,dbMbc[hnBC + 3*(i) + 1],bedci, dbMbc[hnBC + 3*(i) + 1]-bedci );
...
void Fluxsplit(double *hbc, double *Gbc, double *wbc, double *bbc, double *bMbc, double *dbMbc, double *ddbCbc, double *ubc, double *uFDbc,double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc, int CnBC, int Cnbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x, double t, double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        if(RegIndx[i*m1 +  3] == 1)
        {
           ...
           // printf("%d | %d | %d | %d | %d  \n \n",hIbeg,uIbeg,bIbeg,FIbeg,SIbeg   );
            ...
            evolveForce(hbc+ hIbeg,Gbc+ hIbeg,wbc+ hIbeg,bbc + bIbeg,bMbc+ hIbeg,dbMbc+ hIbeg,ddbCbc+ CIbeg,ubc+ uIbeg,uFDbc+ uIbeg,duFDbc+ uIbeg,g,dx,dt,RegIndx[i*m1 +  2],hnBC,hnbc,bnBC,bnbc,unBC,unbc,CnBC,Cnbc,FjhH + FIbeg,FjhG + FIbeg ,SjG + SIbeg,SjHa+ SIbeg,SjGa+ SIbeg,x+ SIbeg,t,a0,a1,a2,a3,a4,a5,a6,a7);
        }
    }
...
void FluxWrap(double *hbc, double *Gbc, double *wbc, double *bbc, double *bMbc, double *dbMbc, double *ddbCbc, double *ubc, double *uFDbc, double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int bnBC, int bnbc, int unBC, int unbc,int CnBC,int Cnbc, int *RegIndx, int m1 ,int RegIndxLen, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *hp, double *Gp)
{
    ...
    Fluxsplit(hbc,Gbc,wbc,bbc,bMbc,dbMbc,ddbCbc,ubc,uFDbc,duFDbc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,CnBC,Cnbc,RegIndx,m1,RegIndxLen,FjhH,FjhG,SjG,SjHa,SjGa,x,t,a0,a1,a2,a3,a4,a5,a6,a7);
...
void ReconandSolve(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double *dbMbeg, double *dbMend,double *ddbCbeg, double *ddbCend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc, int CnBC , int Cnbc,double theta,double dx, double dt, double g, double *Gbc,double *hbc,double *wbc,double *ubc,double *bbc, double *bMbc, double *dbMbc, double *ddbCbc,double *uFDbc, double *duFDbc)
{
    ...
    ReconWrap(h,G,b,hMbeg,hMend,GMbeg,GMend,wMbeg,wMend,bMbeg,bMend,dbMbeg,dbMend,ddbCbeg,ddbCend,uMbeg,uMend,n,hnBC,hnbc,bnBC,bnMBC,bnbc,unBC,unbc,CnBC,Cnbc,theta,dx,dt,g,w,hbc,Gbc,wbc,bbc,bMbc,dbMbc,ddbCbc,RegInd,m1,RegIndLen);
    getufromGsplit(hbc, Gbc, bbc,uMbeg,uMend,dx ,n,u_length ,hnBC,bnBC,hnbc,bnbc,unBC,unbc,RegInd,m1,RegIndLen,ubc);

    uSolveandRecon(h,G,b,hMbeg,hMend,bMbc,bMbc + 3*(n) + hnBC,uMbeg,uMend,duMbeg,duMend,n,hnBC,hnbc,bnBC,bnMBC,bnbc, unBC, unbc,theta,dx,dt,g,uFDbc,duFDbc,RegInd,m1,RegIndLen);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *bMbeg, double *bMend,double *dbMbeg, double *dbMend,double *ddbCbeg, double *ddbCend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,int n, int hnBC , int hnbc, int bnBC, int bnMBC , int bnbc, int unBC , int unbc, int CnBC, int Cnbc,double theta,double dx, double dt, double g, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    FluxWrap(hbc,Gbc,wbc,bbc,bMbc,dbMbc,ddbCbc,ubc,uFDbc,duFDbc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,CnBC,Cnbc,RegInd,m1 ,RegIndLen,x,t,a0,a1,a2,a3,a4,a5,a6,a7,hp,Gp);
    ...
    ReconWrap(hp,Gp,b,hMbeg,hMend,GMbeg,GMend,wMbeg,wMend,bMbeg,bMend,dbMbeg,dbMend,ddbCbeg,ddbCend,uMbeg,uMend,n,hnBC,hnbc,bnBC,bnMBC,bnbc,unBC,unbc,CnBC,Cnbc,theta,dx,dt,g,wp,hpbc,Gpbc,wpbc,bbc,bMbc,dbMbc,ddbCbc,RegIndp,m1,RegIndLen);

    getufromGsplit(hpbc, Gpbc, bbc,uMbeg,uMend,dx ,n,u_length ,hnBC,bnBC,hnbc,bnbc,unBC,unbc,RegIndp,m1,RegIndLen,upbc);

    uSolveandRecon(hp,Gp,b,hMbeg,hMend,bMbc,bMbc + 3*(n) + hnBC,uMbeg,uMend,duMbeg,duMend,n,hnBC,hnbc,bnBC,bnMBC,bnbc, unBC, unbc,theta,dx,dt,g,upFDbc,dupFDbc,RegIndp,m1,RegIndLen);

    FluxWrap(hpbc,Gpbc,wpbc,bbc,bMbc,dbMbc,ddbCbc,upbc,upFDbc,dupFDbc,g,dx,dt,n,hnBC,hnbc,bnBC,bnbc,unBC,unbc,CnBC,Cnbc,RegIndp,m1 ,RegIndLen,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7,hpp,Gpp);
