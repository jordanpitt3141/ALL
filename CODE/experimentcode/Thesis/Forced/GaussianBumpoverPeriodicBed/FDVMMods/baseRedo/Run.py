...
wdir = "../../../../../../../data/2018/raw/Thesis/ForcedFin/Dry/AllNum/FDVM2/"
...
for ki in range(10,11):
    ...
    h,u,G,b = ForcedbedM(x,st,a0,a1,a2,a3,a4,a5,a6,a7,g,dx)
    
    hMbeg = a0*ones(nMBC)
    GMbeg = zeros(nMBC)
    bMbeg = a6*sin(a7*xMbeg)
    dbMbeg = a7*a6*cos(a7*xMbeg)
    wMbeg = hMbeg + bMbeg
    
    uEbeg = zeros(nEBC)
    duEbeg = zeros(nEBC)
    
    ddbCbeg =[ -a7*a7*a6*sin(a7*(x[0] - dx))]
    
    hMend = a0*ones(nMBC)
    GMend = zeros(nMBC)
    bMend = a6*sin(a7*xMend)
    dbMend = a7*a6*cos(a7*xMend)
    wMend = hMend + bMend
    
    uEend = zeros(nEBC)
    duEend = zeros(nEBC)
    
    ddbCend = [ -a7*a7*a6*sin(a7*(x[-1] + dx))]  
    ...
    while t < et: 
        ...
        if close(t,wt,dt):
            ...
            edgevaluesSplit(h_c,G_c,b_c, hMbeg_c,GMbeg_c,wMbeg_c,bMbeg_c,dbMbeg_c,duEbeg_c,uEbeg_c,ddbCbeg_c,hMend_c,GMend_c,wMend_c,bMend_c,dbMend_c,duEend_c,uEend_c, ddbCend_c,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc, hMbc_c,GMbc_c,wMbc_c,bMbc_c,dbMbc_c,duEbc_c,uEbc_c, ddbCbc_c, dx, theta)
        ...
        evolvewrapBC(h_c,G_c,b_c,hMbeg_c,GMbeg_c,wMbeg_c,bMbeg_c,dbMbeg_c,duEbeg_c,uEbeg_c,ddbCbeg_c,hMend_c,GMend_c,wMend_c,bMend_c,dbMend_c,duEend_c,uEend_c,ddbCend_c,hMbeg_c,GMbeg_c,wMbeg_c,duEbeg_c,uEbeg_c,hMend_c,GMend_c,wMend_c,duEend_c,uEend_c,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,dx,dt,g,theta,x_c,t,a0,a1,a2,a3,a4,a5,a6,a7)
        t = t + dt
        print(t)
    ...
    dbMbcC = copyarrayfromC(dbMbc_c,nMbc)
    ...
    for i in range(len(xG)):
        ...
        xMbc.append(xG[i] - 0.5*dx)
        xMbc.append(xG[i])
        xMbc.append(xG[i] + 0.5*dx)
    

    hAbc,uta,GAbc,bAbc,bxAbc,bxxAbc,uxAbc,uxxAbc = ForcedbedMALL(xMbc,t,a0,a1,a2,a3,a4,a5,a6,a7,g,dx)
    wAbc = hAbc + bAbc
    hta,uAbc,Gta,bta,bxta,bxxta,uxta,uxxta= ForcedbedMALL(xEbc,t,a0,a1,a2,a3,a4,a5,a6,a7,g,dx)
    hA,uA,GA,bA,bxA,bxxA,uxA,uxxA = ForcedbedMALL(x,t,a0,a1,a2,a3,a4,a5,a6,a7,g,dx)
