...
void edgevaluesSplit(double *h, double *G,double *bed, double *hMbeg, double *GMbeg, double  *wMbeg, double *bMbeg, double *dbMbeg, double *duEbeg, double *uEbeg, double *ddbCbeg, double *hMend, double *GMend, double  *wMend, double *bMend, double *dbMend, double *duEend, double *uEend, double *ddbCend, int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *dbMbc, double *duEbc, double *uEbc, double *ddbCbc, double dx, double theta)
{
// Take all h,G, bed and calculate all the edge values required for evolve, with split
   ...
   for(j=0; j < n ; j++)
   {
        ...
        dbMbc[nMBC + 3*j] = idx*(bMbc[nMBC + 3*(j+1)] -  bMbc[nMBC + 3*j]);
        dbMbc[nMBC + 3*j + 1] = idx*(bMbc[nMBC + 3*j + 2] -  bMbc[nMBC + 3*j]);
        dbMbc[nMBC + 3*j + 2] = idx*(bMbc[nMBC + 3*j + 2]  -  bMbc[nMBC + 3*j - 1]);
...
void evolveDRY(double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *dbMbc, double *duEbc, double *uEbc, double *ddbCbc, int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, double dx, double dt, double g, double theta, double *hblank, double *Gblank, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    for(i =0; i < n;i++)
    {
		...
		hblank[i] = hMbc[nMBC + 3*i + 1] - dt*idx*(foh - fih) + dt*hS;
		Gblank[i] = GMbc[nMBC + 3*i + 1] - dt*idx*(foG -fiG) + dt*idx*( dx*sourcec) + dt*GS;
...
//UNFINISHED~~~
    ...
    double *dbMbc = malloc((nMbc)*sizeof(double));
    ...
    //update h' and G' 
    edgevaluesSplit(h,G,bed, hMbeg,GMbeg,wMbeg,bMbeg,dbMbeg,duEbeg,uEbeg,ddbCbeg,hMend,GMend,wMend,bMend,dbMend,duEend,uEend,ddbCend,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,hMbc,GMbc,wMbc,bMbc,dbMbc,duEbc,uEbc,ddbCbc,dx,theta);
	evolveDRY(hMbc,GMbc,wMbc,bMbc,dbMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,dx,dt,g,theta,hp,Gp,x,t,a0,a1,a2,a3,a4,a5,a6,a7);

    edgevaluesSplit(hp,Gp,bed, hMbeg1,GMbeg1,wMbeg1,bMbeg,dbMbeg,duEbeg1,uEbeg1,ddbCbeg,hMend1,GMend1,wMend1,bMend,dbMend,duEend1,uEend1,ddbCend,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,hMbc,GMbc,wMbc,bMbc,dbMbc,duEbc,uEbc,ddbCbc,dx,theta);      
	evolveDRY(hMbc,GMbc,wMbc,bMbc,dbMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,dx,dt,g,theta,hpp,Gpp,x,t+ dt,a0,a1,a2,a3,a4,a5,a6,a7);
