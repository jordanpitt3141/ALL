   ...
   int i,j;
    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i*m1 +  3] != 1)
        {
           ...
           for(j=RegInd[i*m1]; j < RegInd[i*m1 +  1] ; j++)
           {
                ...
                wj = h[j]*(h[j] > htol) +  bed[j];
                wjp1 =hjp1*(hjp1 > htol) +  bedjp1;
                wjm1 = hjm1*(hjm1 > htol) + bedjm1;
                dwjf = idx*(wjp1 - wj) ;
                dwjb = idx*(wj - wjm1) ;
    ...
    for (i =0;i<RegIndLen;i++)
    {
        if(RegInd[i*m1 +  3] == 1)
        {
            ...
            double *uN = malloc((RegInd[i*m1 + 2])*sizeof(double));
            getufromG(h + RegInd[i*m1],G + RegInd[i*m1],bed + RegInd[i*m1],uEbc[nEbegI - 1],uEbc[nEendI + 1], hMbc[nMbegI - 2],hMbc[nMendI + 1], bMbc[nMbegI - 2],bMbc[nMendI + 1],dx,RegInd[i*m1 + 2],uN);
...
void evolveDRY(double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *duEbc, double *uEbc, double *ddbCbc, int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, double dx, double dt, double g,double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    i = -1;
    hip1l = hMbc[nMBC + 3*(i + 1)];
    hir = hMbc[nMBC + 3*i  + 2];

    Gip1l = GMbc[nMBC + 3*(i + 1)];
    Gir = GMbc[nMBC + 3*i + 2];

    wip1l = wMbc[nMBC + 3*(i + 1)];
    wir = wMbc[nMBC + 3*i + 2];

    bip1l = bMbc[nMBC + 3*(i + 1)];
    bir = bMbc[nMBC + 3*i + 2];

    ue = uEbc[nEBC + 2*(i) + 1];
    due = duEbc[nEBC + 2*(i) + 1];
	...
	Ger = Gip1l;
	ber = bip1l;
	dber = idx*(bMbc[nMBC + 3*(i + 1) + 2] - bip1l);
	uer  = ue;
	duer = due;
...
void Fluxsplit(double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *duEbc, double *uEbc, double *ddbCbc,int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x, double t, double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7, double dx, double dt, double g)
{

    int MIbeg,EIbeg,CIbeg,Ibeg;
    double hS, GS;
    //Dry First
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        {
            //printf("%d : %d | %d | %d | %d  \n",i,RegIndx[i*m1],RegIndx[i*m1 +  1],RegIndx[i*m1 +  2],RegIndx[i*m1 +  3]    );
            MIbeg = nMBC + 3*(RegIndx[i*m1] - 1);
            EIbeg = (nEBC-1) + 2*(RegIndx[i*m1] - 1);
            CIbeg = nCBC + (RegIndx[i*m1] - 1);

            Ibeg = RegIndx[i*m1];

           // printf("%d | %d | %d | %d | %d  \n \n",hIbeg,uIbeg,bIbeg,FIbeg,SIbeg   );

            evolveDRY(hMbc + MIbeg,GMbc + MIbeg,wMbc + MIbeg,bMbc+ MIbeg,duEbc + EIbeg,uEbc+ EIbeg,ddbCbc+ CIbeg,nMBC,nEBC,nCBC, RegIndx[i*m1 + 2],nMbc,nEbc,nCbc,dx,dt,g,FjhH + Ibeg,FjhG + Ibeg,SjG+ Ibeg,SjHa+ Ibeg,SjGa+ Ibeg,x+ Ibeg,t,a0,a1,a2,a3,a4,a5,a6,a7);
        }
    }
...
void FluxEvo(double *hMbc, double *GMbc,double g, double dx, double dt, int n, int nMBC, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *FjhH, double *FjhG, double *SjG, double *hp, double *Gp,double *SjHa, double *SjGa)
{
    double hS,GS;
    int i;
    double idx = 1.0 / dx;
    for (i = 0; i < n; i++)
    {
        ...
        hp[i] = hMbc[nMBC + 3*(i) + 1] - dt*idx*(FjhH[i+1] - FjhH[i])  + dt*SjHa[i];
        Gp[i] = GMbc[nMBC + 3*(i) + 1] - dt*idx*(FjhG[i+1] - FjhG[i]) + dt*idx*(SjG[i]) + dt*SjGa[i];

    }

}

void FluxWrap(double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *duEbc, double *uEbc, double *ddbCbc,int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc,double g, double dx, double dt, int *RegIndx, int m1 ,int RegIndxLen, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *hp, double *Gp)
{
    ...
    Fluxsplit(hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,RegIndx,m1,RegIndxLen,FjhH,FjhG,SjG,SjHa,SjGa,x,t,a0,a1,a2,a3,a4,a5,a6,a7,dx,dt,g);

    FluxEvo(hMbc,GMbc,g,dx,dt,n,nMBC,x,t,a0,a1,a2,a3,a4,a5,a6,a7,FjhH,FjhG,SjG,hp,Gp,SjHa,SjGa);
...
void ReconValues(double *h, double *G,double *bed, double *hMbeg, double *GMbeg, double  *wMbeg, double *bMbeg, double *duEbeg, double *uEbeg, double *ddbCbeg, double *hMend, double *GMend, double  *wMend, double *bMend, double *duEend, double *uEend, double *ddbCend, int nMBC, int nEBC, int nCBC, int n,int nMbc, int nEbc, int nCbc, double *hMbc, double *GMbc, double *wMbc, double *bMbc, double *duEbc, double *uEbc, double *ddbCbc, double dx, double theta)
{
    ...
    edgevaluesSplit(h,G,bed, hMbeg,GMbeg,wMbeg,bMbeg,duEbeg,uEbeg,ddbCbeg,hMend,GMend,wMend,bMend,duEend,uEend,ddbCend,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,dx,theta,RegInd,m1,RegIndLen);
...
//UNFINISHED~~~
    ...
    int RegIndLen = RegInd[m1-1];
    edgevaluesSplit(h,G,bed, hMbeg,GMbeg,wMbeg,bMbeg,duEbeg,uEbeg,ddbCbeg,hMend,GMend,wMend,bMend,duEend,uEend,ddbCend,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,dx,theta,RegInd,m1,RegIndLen);
    FluxWrap(hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,g,dx,dt,RegInd,m1,RegIndLen,x,t,a0,a1,a2,a3,a4,a5,a6,a7,hp,Gp);
    free(RegInd);

    int *RegIndp = RegSplit(hp,n); 
    int RegIndLenp = RegIndp[m1-1];
    edgevaluesSplit(hp,Gp,bed, hMbeg1,GMbeg1,wMbeg1,bMbeg,duEbeg1,uEbeg1,ddbCbeg,hMend1,GMend1,wMend1,bMend,duEend1,uEend1,ddbCend,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,dx,theta,RegIndp,m1,RegIndLenp);      
	FluxWrap(hMbc,GMbc,wMbc,bMbc,duEbc,uEbc,ddbCbc,nMBC,nEBC,nCBC,n,nMbc,nEbc,nCbc,g,dx,dt,RegIndp,m1,RegIndLenp,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7,hpp,Gpp);
    free(RegIndp);
