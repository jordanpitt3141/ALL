...
#Soliton Problem    
#wdirb = "/home/thanksjoe/Documents/newPHD/data/2018/raw/Thesis/SolitonAgn/FDVM3/"
...
#        os.makedirs(wdir)
    ...
    hm,um,Gm,wta,bta = solitoninit(a0,a1,g,x,0,dx)
    ...
    xshift = x - c*t
    haA,GaA = solitonAveragest0(a0,a1,g,xshift,dx)
    ...
    with open(s,'w') as file2:
        ...
        writefile2.writerow(['dx' ,'dt','time',"cell midpoint" ,'h', 'G' , 'u(m/s)','ha', 'Ga', 'ht','ut' ])        
                   
        for j in range(n):
            writefile2.writerow([str(dx),str(dt),str(t),str(x[j]), str(hmF[j]) , str(GmF[j]) , str(umF[j]), str(haF[j]), str(GaF[j]),str(ht[j]),str(ut[j])])
    """