...
#Soliton Problem    
wdirb = "/home/thanksjoe/Documents/newPHD/data/2018/raw/Thesis/SolitonAgn/FDVM1/"
...
for ki in range(6,20):
    ...
    with open(s,'w') as file2:
        ...
        for j in range(n):
            writefile2.writerow([str(dx),str(dt),str(t),str(x[j]), str(hF[j]) , str(GF[j]) , str(uF[j]),str(ht[j]),str(ut[j])])