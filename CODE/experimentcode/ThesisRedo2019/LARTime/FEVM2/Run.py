...
#Forcing Problem    
wdir = "/home/jp/Documents/PhD/project/data/2019/TimeLAR/FEVM2Long/"
...
for nt in range(trials):
    ...
    with open(s,'w') as file2:
        ...
        writefile2.writerow(['st' ,'et','et-st' ])        
        writefile2.writerow([str(start_time),str(end_time),str(TotTime)])
        
    TotTimes.append(TotTime)

avgTotTime = sum(TotTimes)/len(TotTimes)

s = wdir +  "TimeInfoALL.txt"
with open(s,'w') as file2:
    writefile2 = csv.writer(file2, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)

    writefile2.writerow(['# trials''avg time'])        
    writefile2.writerow([str(len(TotTimes)),str(avgTotTime)])
