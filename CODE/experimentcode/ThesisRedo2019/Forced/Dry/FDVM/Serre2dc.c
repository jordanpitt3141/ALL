    ...
    for(i =0;i<RegLen;i++)
    {
     ...
     for(i =0;i<RegLen;i++)
        ...
        if(RegIndex[i*m +  3] == 1)
        {
            ...
            uIbeg = (unBC-1) + 2*RegIndex[i*m];
            uIend = (unBC-1) + 2*(RegIndex[i*m + 1]);
            
            u0 = ubc[(uIbeg+ 1) - unBC  + 1];
            u1 = ubc[uIend + 1];
            ...
            double *u = malloc((RegIndex[i*m +  2])*sizeof(double));
            
    		getufromGFD(h + RegIndex[i*m],G + RegIndex[i*m],b + RegIndex[i*m],u0,u1,h0,h1,b0,b1,dx,RegIndex[i*m +  2],u);
			...
			for(j=RegIndex[i*m]; j < RegIndex[i*m +  1] ; j++)
           	{
			    ...
			    {
			        ujp1 = u[j - RegIndex[i*m] +1];
			    }
           	
                if(j == RegIndex[i*m])
                {
                    ubc[unBC + 2*(j) - 1] = 0.5*(u[j - RegIndex[i*m] ] + ujm1);
                    ubc[unBC + 2*(j)] = u[j - RegIndex[i*m]];
                    ubc[unBC + 2*(j) + 1] = 0.5*(u[j - RegIndex[i*m]] + ujp1);

                    dubc[unBC + 2*(j) - 1] =idx*(u[j - RegIndex[i*m]] - ujm1);
                    dubc[unBC + 2*(j)] = 0.5*idx*(ujp1 - ujm1);
                    dubc[unBC + 2*(j) + 1] = idx*(ujp1 - u[j - RegIndex[i*m]]);
...
//include BCs
void evolveForce(double *hbc, double *Gbc, double *wbc, double *bMbc, double *dbMbc, double *ddbCbc, double *uFDbc,double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int unBC, int unbc, int CnBC, int Cnbc, double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x, double t, double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7)
{
    double idx = 1.0 / dx;  
	int i;
...
void Fluxsplit(double *hbc, double *Gbc, double *wbc, double *bMbc, double *dbMbc, double *ddbCbc, double *uFDbc,double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int unBC, int unbc, int CnBC, int Cnbc, int *RegIndx, int m1 ,int RegIndxLen, double *FjhH, double *FjhG, double *SjG,double *SjHa, double *SjGa, double *x, double t, double a0, double a1, double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    for (i =0;i<RegIndxLen;i++)
    {
        ...
        if(RegIndx[i*m1 +  3] == 1)
        {
           ...
           // printf("%d | %d | %d | %d | %d  \n \n",hIbeg,uIbeg,bIbeg,FIbeg,SIbeg   );
            ...
            evolveForce(hbc+ hIbeg,Gbc+ hIbeg,wbc+ hIbeg,bMbc+ hIbeg,dbMbc+ hIbeg,ddbCbc+ CIbeg,uFDbc+ uIbeg,duFDbc+ uIbeg,g,dx,dt,RegIndx[i*m1 +  2],hnBC,hnbc,unBC,unbc,CnBC,Cnbc,FjhH + FIbeg,FjhG + FIbeg ,SjG + SIbeg,SjHa+ SIbeg,SjGa+ SIbeg,x+ SIbeg,t,a0,a1,a2,a3,a4,a5,a6,a7);
        }
    }
...
void FluxWrap(double *hbc, double *Gbc, double *wbc, double *bMbc, double *dbMbc, double *ddbCbc, double *uFDbc, double *duFDbc,double g, double dx, double dt, int n, int hnBC, int hnbc, int unBC, int unbc,int CnBC,int Cnbc, int *RegIndx, int m1 ,int RegIndxLen, double *x,double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7, double *hp, double *Gp)
{
    ...
    Fluxsplit(hbc,Gbc,wbc,bMbc,dbMbc,ddbCbc,uFDbc,duFDbc,g,dx,dt,n,hnBC,hnbc,unBC,unbc,CnBC,Cnbc,RegIndx,m1,RegIndxLen,FjhH,FjhG,SjG,SjHa,SjGa,x,t,a0,a1,a2,a3,a4,a5,a6,a7);
...
void evolvewrapForcingANA(double *h, double *G, double *b,double *hMbeg, double *hMend,double *GMbeg, double *GMend,double *wMbeg, double *wMend,double *dbMbeg, double *dbMend,double *ddbCbeg, double *ddbCend, double *uMbeg, double *uMend, double *duMbeg, double *duMend,int n, int hnBC , int hnbc, int unBC , int unbc, int CnBC, int Cnbc,double theta,double dx, double dt, double g, double *x, double t,double a0,double a1,double a2, double a3, double a4, double a5, double a6, double a7)
{
    ...
    FluxWrap(hbc,Gbc,wbc,bMbc,dbMbc,ddbCbc,uFDbc,duFDbc,g,dx,dt,n,hnBC,hnbc,unBC,unbc,CnBC,Cnbc,RegInd,m1 ,RegIndLen,x,t,a0,a1,a2,a3,a4,a5,a6,a7,hp,Gp);
    ...
    uSolveandRecon(hp,Gp,b,hMbeg,hMend,bMbc,bMbc + 3*(n) + hnBC,uMbeg,uMend,duMbeg,duMend,n,hnBC,hnbc, unBC, unbc,theta,dx,dt,g,upFDbc,dupFDbc,RegInd,m1,RegIndLen);

    FluxWrap(hpbc,Gpbc,wpbc,bMbc,dbMbc,ddbCbc,upFDbc,dupFDbc,g,dx,dt,n,hnBC,hnbc,unBC,unbc,CnBC,Cnbc,RegIndp,m1 ,RegIndLen,x,t + dt,a0,a1,a2,a3,a4,a5,a6,a7,hpp,Gpp);
