...
void getufromG(double *h, double *G, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double theta, double dx , int n, int m, int nGhBC,int unBC, int nGhbc, int nubc, double *u, double *hhbc,double *Ghbc)
{
    ...
    for (i =1;i < n-1 ; i++)
    {
        ...
        if((hjmhp < smoothCut &&  hjphm < smoothCut) && (hjmhp != 0   &&  hjphm != 0))
        {

            euhintia11 = ep_S*0.5*dx*((7.0/30)/hjmhp+(1.0/30)/hjphm);

            euhintia12 = ep_S*0.5*dx*((2.0/15)/hjmhp+(0)/hjphm);

            euhintia13 = ep_S*0.5*dx*((-1.0/30)/hjmhp+(-1.0/30)/hjphm);

            euhintia21 = ep_S*0.5*dx*((2.0/15)/hjmhp+(0)/hjphm);

            euhintia22 = ep_S*0.5*dx*((8.0/15)/hjmhp+(8.0/15)/hjphm);

            euhintia23 = ep_S*0.5*dx*((0)/hjmhp+(2.0/15)/hjphm);

            euhintia31 = ep_S*0.5*dx*((-1.0/30)/hjmhp+(-1.0/30)/hjphm);

            euhintia32 = ep_S*0.5*dx*((0)/hjmhp+(2.0/15)/hjphm);

            euhintia33 = ep_S*0.5*dx*((1.0/30)/hjmhp+(7.0/30)/hjphm);
        }
        else
        {
        ...
        LHSa11 = uhintia11 + h3uxintia11 + euhintia11;
        LHSa12 = uhintia12 + h3uxintia12 + euhintia12; 
        LHSa13 = uhintia13 + h3uxintia13 + euhintia13;
        LHSa21 = uhintia21 + h3uxintia21 + euhintia21;
        LHSa22 = uhintia22 + h3uxintia22 + euhintia22; 
        LHSa23 = uhintia23 + h3uxintia23 + euhintia23;
        LHSa31 = uhintia31 + h3uxintia31 + euhintia31;
        LHSa32 = uhintia32 + h3uxintia32 + euhintia32;
        LHSa33 = uhintia33 + h3uxintia33 + euhintia33;
