...
void getufromG(double *h, double *G, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double theta, double dx , int n, int m, int nGhBC,int unBC, int nGhbc, int nubc, double *u, double *hhbc,double *Ghbc)
{
    ...
    for (i =1;i < n-1 ; i++)
    {
        ...
        euhintia11 = ep_S*2*idx*((7.0/30)/hjmhp+(1.0/30)/hjphm);

        euhintia12 = ep_S*2*idx*((2.0/15)/hjmhp+(0)/hjphm);

        euhintia13 = ep_S*2*idx*((-1.0/30)/hjmhp+(-1.0/30)/hjphm);

        euhintia21 = ep_S*2*idx*((2.0/15)/hjmhp+(0)/hjphm);

        euhintia22 = ep_S*2*idx*((8.0/15)/hjmhp+(8.0/15)/hjphm);

        euhintia23 = ep_S*2*idx*((0)/hjmhp+(2.0/15)/hjphm);

        euhintia31 = ep_S*2*idx*((-1.0/30)/hjmhp+(-1.0/30)/hjphm);

        euhintia32 = ep_S*2*idx*((0)/hjmhp+(2.0/15)/hjphm);

        euhintia33 = ep_S*2*idx*((1.0/30)/hjmhp+(7.0/30)/hjphm);
        }
        else
        {
