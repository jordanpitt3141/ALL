...
void convolveC(double *Signal, int SignalLen, double *Kernel, int KernelLen, double *Result)
{
  int n;
...
void convolveS(double *Signal, int SignalLen, double *Kernel, int KernelLen, double *Result)
{
    int n, ns;
    int kmin, kmax, k;
    ...
    for (n = 0; n < SignalLen; n++)
    {
        ...
        if (SignalLen % 2 == 0 &&  n < SignalLen -1 )
        {
            Result[n + 1] = 0;

            kmin = (ns >= KernelLen - 1) ? ns - (KernelLen - 1) : 0;
            kmax = (ns < SignalLen - 1) ? ns : SignalLen - 1;

            for (k = kmin; k <= kmax; k++)
            {
              Result[n + 1] += Signal[k] * Kernel[ns - k];
            }
        }
        else
        {
...
{
    //have to adjust data to be unit offset
    // taken from numerical recipes
    unsigned long n,mmax,m,j,istep,i;
    double wtemp,wr,wpr,wpi,wi,theta;
    double dum,tempr,tempi;
...
void realft0ff(double *data1, unsigned long n, int isign)
{
...
void convlv0ff(double *data1, unsigned long n, double *respns1, unsigned long m,int isign, double *ans1)
{
...
/*void Mollifunc(double *C, double *x, double D, double e, int n, int m)
{
    int i;
    double ie = 1.0 / e;
    for(i=0;i<n/2;i++)
    {
        ...
        if(fabs(x[n/2 - i]*ie) <1)
        {
            C[i] = ie*D*exp(1.0/ ( fabs(x[n/2 - i]*ie)*fabs(x[n/2 - i]*ie) - 1) );
        }
        else
        {
            C[i] = 0;
        ...
        if(fabs(x[n/2 + i]*ie) <1)
        {
            C[m - i] =ie*D*exp(1.0/ ( fabs(x[n/2 + i]*ie)*fabs(x[n/2 + i]*ie) - 1) );
        }
        else
        {
            C[m - i] = 0;
...
void Mollifunc(double *C, double *x, double D, double e, int n)
{
    int i;
    double ie = 1.0 / e;
    for(i=0;i<n/2;i++)
    {
        ...
        if(fabs(x[n/2 + i]*ie) <1)
        {
            C[n - 1 - i] =ie*D*exp(1.0/ ( fabs(x[n/2 + i]*ie)*fabs(x[n/2 + i]*ie) - 1) );
        }
        else
        {
            C[n - 1 - i] = 0;
...
void getufromG(double *h, double *G, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double theta, double dx , int n, int m, int nGhBC,int unBC, int nGhbc, int nubc, double *u, double *hhbc,double *Ghbc, double *MollF, double Molls, int m2, int Molllen)
{
    ...
    double *utemp  = malloc((2*m2)*sizeof(double));
    double *nGisPAD = malloc((m2)*sizeof(double));
...
// last
    ...
    //Mollify nGis
    //convolveS(nGis,m,MollF,m, utemp);
    memcpy(nGisPAD,nGis,(m)*sizeof(double));

    //nGisPAD seems normal
    /*for(i = 0 ; i < m; i++)
    {
        printf("%d | %f | %f \n",i,nGisPAD[i],nGis[i]);
    }*/

    convlv(nGisPAD - 1 ,m2, MollF  - 1 ,Molllen,1, utemp - 1);
  
    double iMolls = 1.0 / Molls;
    memcpy(u,uMbeg, (unBC-1)*sizeof(double));

    for(i = 0 ; i < m; i++)
    {
        u[i + unBC-1] = utemp[i] * iMolls;
    }
    ...
    //nGisPAD causes some memory error
    free(nGisPAD);
    free(utemp);
...
double evolvewrapperADAP(double *G, double *h,double *hMbeg , double *hMend,double *GMbeg , double *GMend,double *uMbeg , double *uMend, double g, double dx, double dt, int n, int nGhBC, int unBC, int nGhhbc, int nubc, double theta, double *hhbc,double *Ghbc,double *ubc, double *Gp, double *hp, double *Gpp, double *hpp,double *MollF, double Molls, int m2, int Molllen)
{
    ...
    double *Moll1 = malloc((m2)*sizeof(double));

    double *Moll2 = malloc((m2)*sizeof(double));

    memcpy(Moll1,MollF, m2*sizeof(double));
    memcpy(Moll2,MollF, m2*sizeof(double));

    //nBCs is number of cells to define ghost cells
    getufromG(h,G,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,theta,dx ,n,u_length ,nGhBC,unBC,nGhhbc,nubc,ubc,hhbc,Ghbc,Moll1,Molls,m2,Molllen);
    ...
    getufromG(hp,Gp,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,theta,dx ,n,u_length ,nGhBC,unBC,nGhhbc,nubc,ubc,hhbc,Ghbc,Moll2,Molls,m2,Molllen);
