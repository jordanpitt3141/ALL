...
g= 9.81
dx = 0.05
x = arange(-50,50 +0.1*dx, dx)  
h,u,G,G1 =DamNreakDRYANA(1,x,5,g)

n = len(x)
m = next_greater_power_of_2(n)
rs = (m-n)
...
xMoll = arange(-10*eta,10*eta +0.1*dx, dx)
xMoll_c = copyarraytoC(xMoll)
Mollifunc(MollC_c, xMoll_c,D,eta,len(xMoll))

for i in range(n):
    
    MollPy[i] = MollifyFunc(D,x[i],eta)

MollC = copyarrayfromC(MollC_c,len(xMoll)) 

uPad = concatenate((u,rs*[0]))


#four1(MollC_c,m, 1)
uPad_c = copyarraytoC(uPad)

stCfft = time()
convlv0ff(uPad_c, m, MollC_c,len(xMoll),1, uCONV_c)
uconvC = copyarrayfromC(uCONV_c,2*m) 
uconvC = uconvC 
uPadfft = copyarrayfromC(uPad_c,m) 
etCfft = time()
tCfft = etCfft - stCfft


stPYstd = time()
uMpy = signal.convolve(u, MollPy, mode='same')
etPYstd = time()
tPYstd = etPYstd -stPYstd
...
m2 = next_greater_power_of_2(m)  


xubc_c = copyarraytoC(xubc)
...
Mollen = len(xMoll) 
MollC_c = mallocPy(Mollen)
Mollifunc(MollC_c, xMoll_c,D,eta,Mollen )

MollC = copyarrayfromC(MollC_c,Mollen) 
Molls = sum(MollC)
MollCPAD = concatenate((MollC, [0]*(m2 - Mollen)))

MollCPAD_c = copyarraytoC(MollCPAD)
...
while ct < endt:
    ...
    #evolvewrapperconsistenttime(G_c, h_c,hMbeg_c , hMend_c,GMbeg_c ,GMend_c,uMbeg_c,uMend_c,g,dx, dt,n,GhnBC,unBC,nGhhbc,nubc,theta, hhbc_c,Ghbc_c,ubc_c,Gp_c,hp_c, Gpp_c,hpp_c)
    dt = evolvewrapperADAP(G_c, h_c,hMbeg_c , hMend_c,GMbeg_c ,GMend_c,uMbeg_c,uMend_c,g,dx, dt,n,GhnBC,unBC,nGhhbc,nubc,theta, hhbc_c,Ghbc_c,ubc_c,Gp_c,hp_c, Gpp_c,hpp_c,MollCPAD_c,Molls,m2,Mollen)
...
getufromG(h_c, G_c,hMbeg_c,hMend_c,GMbeg_c,GMend_c,uMbeg_c,uMend_c,theta,dx,n,2*n +1,GhnBC,unBC,nGhhbc,nubc,ubc_c,hhbc_c,Ghbc_c,MollCPAD_c,Molls,m2, Mollen)
ubcC = copyarrayfromC(ubc_c,nubc)
