...
def copyarrayfromC2D(a,n,m):
    b = zeros((n,m),dtype=int)
    for i in range(n):
        for j in range(m):
            b[i][j] =  readfrom2DmemI(a,i,j)
       
    return b
    
def copyarrayfromCINT(a,n):
    b = [0]*n
    for i in range(n):
        b[i] = readfrommemINT(a,i)
...
def DryRegTest(x):
    ...
    G = getGfromupy(h,u,bed,u[0],u[-1],h[0],h[-1],0,0,dx)        
    return h,u,G
...
regn = readfrom2DmemI(A_C,0,0)

AC =copyarrayfromC2D(A_C,regn,5)

getufromGSEP(h_c, G_c, hMbeg_c ,hMend_c,GMbeg_c , GMend_c,uMbeg_c, uMend_c,1,dx ,n, 2*n + 1, GhnBC,unBC, nGhhbc, nubc, ubc_c , hhbc_c,Ghbc_c, A_C, regn, 5)

ubcC = copyarrayfromC(ubc_c,nubc)
hbcC = copyarrayfromC(hhbc_c,nGhhbc)
GbcC = copyarrayfromC(Ghbc_c,nGhhbc) 
"""

#Solver
#So our solver solves the analytic soliton problem with second order accuracy in h, u and G. 
"""
wdir = "../../../../../data/raw/DryTestREG3/Solver/Soliton/theta1/"
...
hhbC = copyarrayfromC(hhbc_c,nGhhbc)
GhbC = copyarrayfromC(Ghbc_c,nGhhbc)
ubcC = copyarrayfromC(ubc_c,nubc)
