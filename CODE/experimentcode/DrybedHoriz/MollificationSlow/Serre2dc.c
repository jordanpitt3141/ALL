...
void four1(double *data1, unsigned long nn, int isign)
{
...
void realft(double *data1, unsigned long n, int isign)
{
...
void convlv(double *data1, unsigned long n, double *respns1, unsigned long m,int isign, double *ans1)
{
    ...
    unsigned long i,no2;
    double dum,mag2,ino2;

    /*double *fft1 = malloc((n<<1)*sizeof(double));

    double *fft= fft1 -1;*/

    // Dont need to pad zeros if its the same length
    /*
    for (i=1;i<=(m-1)/2;i++)
    ...
    */
    ...
    //We only need tyo update data's FFT
    ...
    {
        if (isign == 1) {
            ans[i-1]=(data[i-1]*(dum=respns[i-1])-data[i]*respns[i])*ino2;
            ans[i]=(data[i]*dum+data[i-1]*respns[i])*ino2;
        } 
        else if (isign == -1) 
        {
            ...
            ans[i-1]=(data[i-1]*(dum=respns[i-1])+data[i]*respns[i])/mag2*ino2;
            ans[i]=(data[i]*dum-data[i-1]*respns[i])/mag2*ino2;
...
void Mollifunc(double *C, double *x, double D, double e, int n)
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        if(fabs(x[i]*ie) <1)
        {
            C[i] = ie*D*exp(1.0/ ( fabs(x[i]*ie)*fabs(x[i]*ie) - 1) );
        }
        else
        {
            C[i] = 0;
...
void getufromG(double *h, double *G, double *hMbeg, double *hMend, double *GMbeg, double *GMend,double *uMbeg, double *uMend,double theta, double dx , int n, int m, int nGhBC,int unBC, int nGhbc, int nubc, double *u, double *hhbc,double *Ghbc, double *MollF, double Molls)
{
...
double evolvewrapperADAP(double *G, double *h,double *hMbeg , double *hMend,double *GMbeg , double *GMend,double *uMbeg , double *uMend, double g, double dx, double dt, int n, int nGhBC, int unBC, int nGhhbc, int nubc, double theta, double *hhbc,double *Ghbc,double *ubc, double *Gp, double *hp, double *Gpp, double *hpp,double *MollF, double Molls)
{
    ...
    //nBCs is number of cells to define ghost cells
    getufromG(h,G,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,theta,dx ,n,u_length ,nGhBC,unBC,nGhhbc,nubc,ubc,hhbc,Ghbc,MollF,Molls);
    ...
    getufromG(hp,Gp,hMbeg,hMend,GMbeg,GMend,uMbeg,uMend,theta,dx ,n,u_length ,nGhBC,unBC,nGhhbc,nubc,ubc,hhbc,Ghbc,MollF,Molls);
