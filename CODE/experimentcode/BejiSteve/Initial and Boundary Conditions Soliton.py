...
def sech2(x):
    """
    sech2 : computes sech^2(x)
    
    Input:
        x       : where to evaltude sech^2
...
def soliton(x,t,g,a0,a1):
    ...
    """
    sech2 : computes the h(x,t) at location x and time t for the soliton initial conditions
    
    Input:
    x       : location
    t       : time
    g       : acceleration due to gravity
    a0      : quiescent water depth
    a1      : ampltide of soliton
    
    Output:
        h(x,t) for the soltion
    """
...
#Soliton Over Slope Experiment
dx = 0.005

#This is a lower dt then required by CFL condition, ensures that our times are nice
dt = 0.1*dx

#Left boundary location, want it far enough away so we get no strange effects
startx = -350

#Right boundary location , want it far enough away so we get no strange effects 
endx = 250.0
...
h,u,bed = soloverslope(x,a0,a1,slopbeg,slopend,topbed,g)
