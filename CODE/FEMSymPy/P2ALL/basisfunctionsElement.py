...
def ExpressionConvertOp(s,hyperlist,x,Cterm,Ctermname,Clb,Cub,filen):
    # hpyerlist has is a list of tuples of (char,functionlist,functionname)
    n = len(hyperlist)
    if (len(s) == 0):
        #can do the interesting stuff here, write out to file
        ...
        for i in range(n):
            if(s[-1] == hyperlist[i][0]):
                m = len(hyperlist[i][1])
                for j in range(m):
                    ExpressionConvertOp(s[:-1],hyperlist,x,hyperlist[i][1][j][0],[hyperlist[i][2][j]],Clb,Cub,filen)
    else:
        for i in range(n):
            if(s[-2] == hyperlist[i][0]):
                m = len(hyperlist[i][1])
                
                if(s[-1] == '*'):
                    for j in range(m):
                        ExpressionConvertOp(s[:-2],hyperlist,x,Cterm*hyperlist[i][1][j][0],[hyperlist[i][2][j]] + Ctermname,Clb,Cub,filen) 
                if(s[-1] == '/'):
                    for j in range(m):
                        ExpressionConvertOp(s[:-2],hyperlist,x,Cterm/hyperlist[i][1][j][0],[hyperlist[i][2][j]] + Ctermname,Clb,Cub,filen) 
...
Dphijm1o2 = quadcontEM(x,xjm1o2,xj,xjp1o2)
Dphij = quadcontM(x,xjm1o2,xj,xjp1o2)
Dphijp1o2 = quadcontEP(x,xjp1o2,xj,xjm1o2)
...
Ddphijm1o2 = phideriv(x,Dphijm1o2)
Ddphij = phideriv(x,Dphij )
Ddphijp1o2 = phideriv(x,Dphijp1o2)
...
DphisNAME = ["Dphijm1o2","Dphij","Dphijp1o2"]
DdphisNAME = ["Ddphijm1o2","Ddphij","Ddphijp1o2"]
...
Dphis = [Dphijm1o2,Dphij,Dphijp1o2] 
Ddphis = [Ddphijm1o2,Ddphij,Ddphijp1o2]
...
hyperlist = []
hyperlist.append( ('f',Dphis,DphisNAME))
hyperlist.append( ('g',Ddphis,DdphisNAME))
...
with open(s,'a') as file2:
     ...
     ExpressionConvert('fp',hyperlist,x,None,None,-1,1,writefile2)      
...
with open(s,'a') as file2:
     ...
     ExpressionConvert('fpp',hyperlist,x,None,None,-1,1,writefile2)  
...
with open(s,'a') as file2:
     ...
     ExpressionConvert('ffdd',hyperlist,x,None,None,-1,1,writefile2)  
...
with open(s,'a') as file2:
     ...
     writefile2.writerow(['Integrand','lower bound of integral','upper bound of integral', 'Value of Integral', 'W Term 1', 'W Term 2' , 'W Term 3' , 'D Term 1', 'D Term 2'])        
     ExpressionConvert('fgdd',hyperlist,x,None,None,-1,1,writefile2)      
"""
