...
wdir = "../../../../../data/ThesisRAW/FullDispRelImag/U" + str(u) + "/h" + str(h) + "/k"+ str(k) + "/"
...
for i in range(n):  
    ...
    erro11[i] = abs(o11s[i].imag)  /abs(w1)
    erro21[i] = abs(o21s[i].imag)  /abs(w1) 
    errof21[i] = abs(of21s[i].imag)     /abs(w1)
    erro31[i] = abs(o31s[i].imag)  /abs(w1)
    
    erroD21[i] = abs(oD21s[i].imag)  /abs(w1)
    
    erroW21[i] = abs(oW21s[i].imag) /abs(w1)
