...
wdir = "../../../../../data/ThesisRAW/FullDispRelReal/U" + str(u) + "/h" + str(h) + "/k"+ str(k) + "/"
...
dxs = linspace(10**(-6),0.2*wavel,num=2000)   
n = len(dxs)
w1s = zeros(n)
w2s = zeros(n) 
...
for i in range(n):  
    ...
    erro11[i] = abs(-w1 - o11s[i].real)  /abs(w1)   
    erro21[i] = abs(-w1  - o21s[i].real)  /abs(w1)
    errof21[i] = abs(-w1 - of21s[i].real)  /abs(w1)    
    erro31[i] = abs(-w1 - o31s[i].real)  /abs(w1)
    
    erroD21[i] = abs(-w1  - oD21s[i].real)  /abs(w1)
    
    erroW21[i] = abs(-w1  - oW21s[i].real)  /abs(w1)
