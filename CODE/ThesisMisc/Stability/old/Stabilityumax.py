...
from scipy.linalg import norm, eig,det
...
hS = [1]#linspace(10**(-15),10,num=10) 
kS = linspace(10**(-15),10**3,num=1000) 
uS = [1]#linspace(-10,10,num=10)
g=9.81
...
for h in hS:
    for k in kS:
        for u in uS:
            ...
            dxs = [0.1]#linspace(10**(-15),1.0,num=100)  
            n1 = len(dxs)
            ...
            for i in range(n1):
                ...
                m,detm = naiveFD(k,g,u,h,dx,dt)
                q,detq = LWN(k,g,u,h,dx,dt)
                #q = LWN(k,g,u,h,dx,dt)
            
                Ms.append(m)
                Qs.append(q)

            #print(h,k,u,dx,max(Qs))
            maxQs.append(max(Qs))
            detqs.append(abs(detq))
            detms.append(abs(detm))
