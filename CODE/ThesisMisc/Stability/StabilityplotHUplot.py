...
ks = [0.1,1,10,100]
us = linspace(ubeg+ tol,uend,num=100,dtype=complex)  
Hs = linspace(hbeg + tol,hend,num=100,dtype=complex)  

for dx in dxs :
     for k in ks:
    
        sdir = "../../../../../data/ThesisRAW/StabilityFixed/Cr0p5PicsUHmedHlargeU4/"   + "dx"+str(dx) +"/"
        ...
        xlabel("H")
        ylabel("U")
        plot(phs,pks,'.',label="stable")
        plot(phsn,pksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "o1k" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf()

        xlabel("H")
        ylabel("U")        
        plot(rhs,rks,'.',label="stable")
        plot(rhsn,rksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "o2k" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf()

        xlabel("H")
        ylabel("U")      
        plot(bhs,bks,'.',label="stable")
        plot(bhsn,bksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "So2k" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf()

        xlabel("H")
        ylabel("U")         
        plot(shs,sks,'.',label="stable")
        plot(shsn,sksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "o2FEMk" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf()    

        xlabel("H")
        ylabel("U")          
        plot(nhs,nks,'.',label="stable")
        plot(nhsn,nksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "o3k" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf() 
    
        xlabel("H")
        ylabel("U")    
        plot(mhs,mks,'.',label="stable")
        plot(mhsn,mksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "nFDk" + str(k)  + ".png"
        savefig(s, bbox_inches='tight')
        clf() 
 
        xlabel("H")
        ylabel("U")        
        plot(qhs,qks,'.',label="stable")
        plot(qhsn,qksn,'.r',label="unstable")
        legend(loc=2)
        xlim([hbeg,hend])
        ylim([ubeg,uend])
        s = sdir + "LWk" + str(k) + ".png"
        savefig(s, bbox_inches='tight')
        clf() 
